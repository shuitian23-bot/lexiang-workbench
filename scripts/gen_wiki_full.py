...
=======
        _sub_slim = [{'slug':a['slug'],'title':a['title'],'cat':a['cat'],'type':a.get('type',''),'desc':(a.get('desc') or '')[:40],'sku':(re.search(r'-(\d+)\.html',a['slug']).group(1) if re.search(r'-(\d+)\.html',a['slug']) else '')} for a in sub_articles]
>>>>>>> 8c25cec (perf(search): slim desc 120→40 + 预压.gz + nginx gzip_static, 修搜索29s慢)
...
=======
    ...
    # 生成 slim 和 recent
    _slim = [{'slug':a['slug'],'title':a['title'],'cat':a['cat'],'type':a.get('type',''),'desc':(a.get('desc') or '')[:40],'sku':(re.search(r'-(\d+)\.html',a['slug']).group(1) if re.search(r'-(\d+)\.html',a['slug']) else '')} for a in all_articles]
>>>>>>> 8c25cec (perf(search): slim desc 120→40 + 预压.gz + nginx gzip_static, 修搜索29s慢)
