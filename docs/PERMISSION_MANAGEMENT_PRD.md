...
2026-09-08 已确认：新权限申请在“填写信息”中必须单选业务负责人，默认空值；按“线上数据权限”提供 zhangjq4（消费业务 to C）、huangjq5（商用业务 to B/b），按“产品/IT”提供 zhangxy43（to C 相关）、zhangrui32（to B/b 相关）、zhangyi44（乐享相关）。不得自由输入，不再根据角色推导审批人。首次访问、创建账号、权限变更和用户管理创建/权限变更统一适用；基本资料直接保存、启停和重置密码不新增此要求。角色管理的负责人仍作为角色维护信息保留。历史申请按已有审批快照流转，不自动改派。
...
| 业务负责人 | 对申请权限与业务归属负责 | 审批整张申请并填写所属组织 | 新申请由提交者从固定名单单选；历史多人会签必须全部通过 |
...
2. 角色详情包含功能权限、数据权限两个页签；功能权限沿用“一级目录 → 页面 → 功能”，普通数据权限使用“业务目录 → 数据源 → 授权项”，前两层仅展开/收起，第三层勾选。
...
1. 所有普通授权选择与回显统一为“业务目录 → 数据源 → 授权项”三层；前两层只负责展开/收起，第三层才展示复选框。
2. 一级目录与数据源管理绑定的业务目录一致；第二层为运营指标查询、订单状态查询、信源引用查询、会员标签查询、线索池查询。前两层默认展开，可独立折叠。
3. 第三层直接显示授权项名称；保留已有 ID 和含义，新增订单状态 mock 范围。不改变自定义授权模型。
4. 每个一级目录右侧都有搜索图标；点击后展开当前目录、显示搜索框并自动聚焦。
5. 搜索按当前目录的数据源或授权项名称实时模糊匹配；结果保留完整父级路径，各目录搜索状态互不影响。
6. 搜索、无结果、关闭搜索不能丢失已选数据；关闭搜索同时清空当前目录关键词。
7. 无结果时仅在当前目录显示“暂无匹配的数据源或授权项”。
...
| 场景 | 节点顺序 |
| --- | --- |
| 内部用户为自己申请权限 | 申请人直线经理 → 所选业务负责人 → 系统自动生效 |
| 内部用户为其他内部用户申请 | 申请人直线经理 → 被申请人直线经理 → 所选业务负责人 → 系统自动生效 |
| 内部用户为外部用户申请/创建账号 | 关联人（与申请人相同时省略）→ 申请人直线经理 → 所选业务负责人 → 系统自动生效 |
| 外部用户为自己申请 | 关联人 → 所选业务负责人 → 系统自动生效 |
| 外部用户为其他外部用户申请 | 被申请人关联人 → 所选业务负责人 → 系统自动生效 |
| 权限申请中的启用/禁用 | 系统管理员审批 → 系统执行 |

普通权限变更不生成系统管理员最终审批节点。即使角色为空、仅申请数据权限，也必须由所选业务负责人审批；负责人缺失、非法或尚未审批通过时不得自动生效。
...
2. 关联人审批：确认或驳回外部人员关系。
3. 申请人/被申请人直线经理审批：可在审批前调整租户、添加角色、复制他人角色、选择数据权限；调整后更新最终权限快照，保留申请时选定的业务负责人，不因角色变化重新派人。
4. 业务负责人审批：权限范围只读；同意时必须选择本申请的所属组织，可填写意见。
5. 审批结果必须选择“同意”或“驳回”；意见是否必填由第 20 章待评审项确定。
6. 驳回后状态为“已驳回/申请人修改”，不执行任何权限变更。

### 11.4 单一业务负责人及历史会签兼容

新申请只生成一项业务审批任务，使用填写信息时选定人的 ITCode 快照；新增/复制/移除角色不会改变此人。其审批结果覆盖整张申请，选择所属组织后通过才整体生效。缺失或非法值不能提交，也不能在最终执行时跳过审批。以下规则仅适用于已有多负责人历史申请：
...
3. 数据权限分为普通授权与自定义授权；普通授权使用业务目录 → 数据源 → 授权项三级树，自定义授权沿用现有配置，不受层级调整影响。
...
3. 数据权限：普通授权按“业务目录 → 数据源 → 授权项”，前两层仅展开且默认展开，支持目录内搜索；自定义授权沿用现有配置。
...
4. 普通数据权限展示业务目录、数据源、授权项三级；前两层只展开，第三层勾选，各目录独立搜索。数据源与数据源管理共用定义，详见 UI_SPEC 的 SPEC-13。
5. 复制只包含角色、角色功能/数据和用户单独数据，不包含租户、组织、账号或用户级功能。
6. 权限变更最终无变化不能提交；租户、角色、有效功能或数据发生变化时可以提交。
7. 五种填写信息组合的字段、顺序、必填、自动带出、提交载荷和审批回显一致。
8. 新申请必须由所选唯一业务负责人通过后才整体生效；历史多业务负责人申请仍需全部通过。任一驳回不产生部分权限。
...
| 普通数据权限三级树与搜索 | `PermissionDataDirectoryList.vue`、P13 验收 | 以 UI_ACCEPTANCE 本轮验收记录为准 |
