...
assert.deepEqual(dataDirectories.map((directory) => directory.name), ['乐享运营', 'GEO 看板', '在职员工管理', '企业客户管理'], '数据权限一级目录必须与门户工作台目录一致')
assert.deepEqual(dataDirectories.flatMap((directory) => directory.sources.flatMap((source) => source.datasets.map((dataset) => dataset.id))).sort(), catalog.dataPermissions.map((permission) => permission.id).sort(), '一级目录化不得丢失或重复数据权限')
assert.equal(dataDirectories.some((directory) => directory.sources.some((source) => source.datasets.some((dataset) => dataset.name.includes('·')))), false, '数据集名称不得拼接目录或类型前缀')
assert.ok(dataDirectories.find((directory) => directory.name === '乐享运营')?.sources.some((source) => source.datasets.some((dataset) => dataset.name === '华东区')), '数据源下必须使用原始授权项名称')
...
assert.ok(dataDirectorySource.includes("dataset.name.toLowerCase().includes(keyword)"), '目录内搜索必须支持当前目录授权项名称过滤')
assert.ok(dataDirectorySource.includes('暂无匹配的授权项') && dataDirectorySource.includes('暂无匹配的数据源或授权项') && dataDirectorySource.includes("searchKeywords[id] = ''"), '目录搜索必须覆盖局部空态和关闭清空')
