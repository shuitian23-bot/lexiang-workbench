<template>
  <div class="permission-page-vue">
              ...
              <table v-if="filteredManagedFunctions.length" class="permission-table function-table">
                ...
                <tbody>
                  ...
                  >
                      ...
                      <div :class="['function-name-cell', 'function-tree-name', { nested: item.depth > 0 }]" :style="{ '--tree-depth': item.depth }">
                        ...
                        <div class="function-tree-copy">
                          <div class="function-tree-title">
                            <span v-if="item.itemKind !== 'function'" :class="['function-node-icon', item.itemKind]" aria-hidden="true"></span>
                            <b>{{ item.name }}</b>
                    ...
                    <td><span :class="['function-type-badge', 'function-hierarchy-badge', item.itemKind]">{{ functionObjectTypeLabel(item) }}</span></td>
    ...
    <div v-if="picker.visible" class="permission-modal" @click.self="closePicker">
      <div v-modal-overflow-state class="modal-panel small">
    ...
    <div v-if="roleCardDetail.visible && roleCardDetailRole" class="permission-modal permission-detail-layer" @click.self="closeRoleCardDetail">
      <div v-modal-overflow-state class="modal-panel role-card-detail-modal">
    ...
    <div v-if="functionMenuEditor.visible" class="permission-modal" @click.self="closeFunctionMenuEditor">
      <div v-modal-overflow-state class="modal-panel function-menu-editor-modal function-create-modal">
    ...
    <div v-if="functionEditor.visible && functionEditor.mode === 'edit'" class="permission-modal" @click.self="closeFunctionEditor">
      <div v-modal-overflow-state class="modal-panel function-editor-modal">
    ...
    <div v-if="dataSourceEditor.visible" class="permission-modal" @click.self="closeDataSourceEditor">
      <div v-modal-overflow-state class="modal-panel datasource-editor-modal">
    ...
    <div v-if="approvalWorkspace.visible && activeApproval" class="permission-modal" @click.self="closeApprovalWorkspace">
      <div v-modal-overflow-state class="modal-panel approval-workspace-modal">
        ...
        <div class="approval-workspace-body" data-modal-scroll-region>
    ...
    <div v-if="approvalNotificationModal.visible && submittedApproval" class="permission-modal" @click.self="closeApprovalNotificationModal">
      <div v-modal-overflow-state class="modal-panel approval-notification-modal simple-notice-modal">
      ...
      <div v-modal-overflow-state class="modal-panel role-editor-modal">
        ...
        <div class="role-editor-head">
          ...
          <div v-if="roleEditor.draft.sensitivity" class="modal-header-status"><span :class="['sensitivity-badge', sensitivityRisk(roleEditor.draft.sensitivity)]">{{ sensitivityLabel(roleEditor.draft.sensitivity) }}</span></div>
        </div>
        ...
        <section v-if="roleEditor.activeTab === 'basic'" class="role-editor-section" data-modal-scroll-region>
        ...
        <section v-else-if="roleEditor.activeTab === 'function'" class="role-editor-section" data-modal-scroll-region>
          <div class="permission-subhead custom-rule-head">
            <div>
              <b>功能权限</b>
        ...
        <section v-else class="role-editor-section" data-modal-scroll-region>
    ...
    <div v-if="userWorkspace.visible" class="permission-modal" @click.self="closeUserWorkspace">
      <div v-modal-overflow-state class="modal-panel role-editor-modal user-editor-modal">
        ...
        <div class="role-editor-head">
          ...
          <div v-if="userWorkspace.draft" class="modal-header-status"><span class="table-status user-modal-status" :class="userWorkspace.draft.statusKey">{{ userStatusLabel(userWorkspace.draft) }}</span></div>
        </div>
        ...
        <section v-if="userWorkspace.draft && userWorkspace.activeTab === 'basic'" class="role-editor-section user-basic-section" data-modal-scroll-region>
        ...
        <section v-else-if="userWorkspace.draft && userWorkspace.activeTab === 'roles'" class="role-editor-section" data-modal-scroll-region>
          <div class="permission-subhead custom-rule-head">
            <div>
              <b>已分配角色</b>
        ...
        <section v-else-if="userWorkspace.draft && userWorkspace.activeTab === 'data'" class="role-editor-section" data-modal-scroll-region>
        ...
        <section v-else-if="userWorkspace.draft && userWorkspace.activeTab === 'history'" class="role-editor-section" data-modal-scroll-region>
        ...
        <section v-else-if="userWorkspace.draft && userWorkspace.activeTab === 'login'" class="role-editor-section" data-modal-scroll-region>
          <div class="permission-subhead custom-rule-head">
            <div>
              <b>登录日志</b>
    ...
    <div v-if="userStatusConfirm.visible && statusTargetUser" class="permission-modal" @click.self="closeUserStatusConfirm">
      <div v-modal-overflow-state class="modal-panel status-confirm-modal direct-status-modal">
      ...
      <div v-modal-overflow-state class="modal-panel">
        ...
        <div class="record-list">
          <article v-for="record in records" :key="record.time + record.title">
            ...
            <b>{{ record.title }}</b>
            <p>{{ record.detail }}</p>
            <template v-if="record.releaseKey">
              <p v-for="environment in ['new', 'formal']" :key="environment">
                {{ environment === 'new' ? 'new 预览' : '正式环境' }}：
                <template v-if="permissionReleaseLedger.records[record.releaseKey]?.releases[environment]">
                  {{ permissionReleaseLedger.records[record.releaseKey].releases[environment].publisher }} ·
                  {{ permissionReleaseLedger.records[record.releaseKey].releases[environment].releasedAt }} ·
                  {{ permissionReleaseLedger.records[record.releaseKey].releases[environment].version }}
                </template>
                <template v-else>暂无发布记录</template>
              </p>
            </template>
...
const requestTypes = [
  { key: 'change', no: '01', label: '权限变更', summary: '已有账号新增或调整菜单、功能、数据和 Skill 权限。', route: '必要关系确认 + 直线经理 + 所选业务负责人' },
  { key: 'create', no: '02', label: '创建账号', summary: '关联人为外部协作人员创建账号并同步申请权限。', route: '申请人直线经理 + 所选业务负责人' },
...
const records = ref([
  {
    ...
    releaseKey: 'permissions-guanfeng2-20260910'
  },
  {
...
function functionObjectTypeLabel(item) {
  if (!item || item.itemKind === 'function') return functionTypeLabel(item?.type || '')
  const levels = ['一级', '二级', '三级', '四级']
  const level = levels[item.depth] || `${item.depth + 1} 级`
  return `${level}${item.itemKind === 'directory' ? '目录' : '菜单'}`
}
...
onMounted(() => {
  ...
  loadPocReleaseLedger().then((ledger) => { permissionReleaseLedger.value = ledger })
...
.modal-panel:not(.role-editor-modal):not(.approval-workspace-modal):not(.simple-notice-modal) > .modal-actions {
  margin-right: -24px;
  margin-bottom: -24px;
  margin-left: -24px;
  padding: 12px 24px;
...
.modal-panel.modal-content-overflowing > .modal-actions {
  position: sticky;
  bottom: 0;
  z-index: 4;
}

.modal-panel.modal-content-can-scroll-down > .modal-actions {
...
.role-editor-modal {
  --modal-close-offset: 14px;
  --modal-close-size: 34px;
  --modal-status-gap: 16px;
  display: flex;
  flex-direction: column;
...
.role-editor-modal .role-editor-head {
  padding-right: calc(var(--modal-close-offset) + var(--modal-close-size) + var(--modal-status-gap));
}

.role-editor-head > div {
  min-width: 0;
}
...
.function-hierarchy-badge {
  border: 1px solid transparent;
  white-space: nowrap;
}

.function-hierarchy-badge.directory {
  border-color: var(--color-primary-border);
  background: var(--color-primary-subtle);
  color: var(--color-primary);
}

.function-hierarchy-badge.menu {
...
.function-tree-row.directory:not(.active) {
  box-shadow: inset 3px 0 0 var(--color-primary-border);
}

.function-tree-row.menu .function-node-icon {
  color: var(--color-text-secondary);
}
...
.function-node-icon.directory::before {
  content: '';
  position: absolute;
  right: 1px;
  bottom: 2px;
...
.function-node-icon.directory::after {
  content: '';
  position: absolute;
  top: 2px;
  left: 3px;
...
.function-node-icon.menu::before {
  content: '';
  position: absolute;
  top: 1px;
  left: 3px;
