<template>
  <div class="pipeline-quality-native" v-html="qualityHtml"></div>
</template>
...
declare global {
  interface Window {
    ...
    qualityApplyFilter?: () => void
    qualityClearFilter?: () => void
    qualityRefresh?: () => void
    switchFirstTokenMode?: (mode: string) => void
    switchWaitLenMode?: (mode: string) => void
  }
}
...
onMounted(async () => {
  ...
  window.qualityRefresh?.()
})

onBeforeUnmount(() => {
  document.querySelectorAll('.pipeline-quality-native [_echarts_instance_]').forEach((el) => {
    try {
...
function ensureECharts() {
  ...
  return new Promise<void>((resolve, reject) => {
    const existing = document.querySelector<HTMLScriptElement>('script[data-echarts-runtime="true"]')
    if (existing) {
    ...
    script.dataset.echartsRuntime = 'true'
...
async function ensureQualityRuntime() {
  if (typeof window.qualityRefresh === 'function') return
  await new Promise<void>((resolve, reject) => {
    const existing = document.querySelector<HTMLScriptElement>('script[data-quality-runtime="true"]')
    if (existing) {
      existing.addEventListener('load', () => resolve(), { once: true })
      if (typeof window.qualityRefresh === 'function') resolve()
      return
    }
    const script = document.createElement('script')
    script.src = `${import.meta.env.BASE_URL}admin-runtime/workbench-quality.js`
    script.dataset.qualityRuntime = 'true'
    script.onload = () => resolve()
    script.onerror = () => reject(new Error('Failed to load quality runtime'))
