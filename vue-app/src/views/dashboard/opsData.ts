export type OpsRangeKey = '1d' | '7d' | '14d' | '30d' | 'custom'
...
export function rowIso(value: string) {
  const [month, day] = value.split('/')
  return `2026-${month.padStart(2, '0')}-${day.padStart(2, '0')}`
}

export function rowsForRange(rows: OpsRow[], range: OpsRangeKey, start: string, end: string) {
  if (range === 'custom') {
    const from = start <= end ? start : end
    const to = start <= end ? end : start
    return rows.filter(row => rowIso(row.d) >= from && rowIso(row.d) <= to)
  }
  const size = range === '1d' ? 1 : range === '7d' ? 7 : range === '14d' ? 14 : 30
  return rows.slice(-size)
}

export function sumRows(rows: OpsRow[], key: keyof OpsRow) {
  return rows.reduce((total, row) => total + (Number(row[key]) || 0), 0)
}

export function avgRows(rows: OpsRow[], key: keyof OpsRow) {
  return rows.length ? Math.round(sumRows(rows, key) / rows.length) : 0
}

export function fmtW(value: number) {
  return value >= 10000 ? `${(value / 10000).toFixed(1)}万` : Math.round(value).toLocaleString()
}

export function fmtY(value: number) {
...
export function rangeLabel(range: OpsRangeKey) {
  return ({ '1d': '最近1天', '7d': '最近7天', '14d': '最近14天', '30d': '最近30天', custom: '自定义' })[range]
}

export function metricDelta(rows: OpsRow[], key: keyof OpsRow) {
...
export function percentValue(part: number, total: number, min = 4) {
  if (!total) return 0
