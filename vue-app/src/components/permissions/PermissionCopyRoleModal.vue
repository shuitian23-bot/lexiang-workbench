<template>
  <div v-if="visible" class="permission-modal permission-scope-picker-modal" @click.self="$emit('close')" @keydown.esc="$emit('close')">
    <section ref="dialog" v-modal-overflow-state="adaptiveFooter" :class="{ 'permission-overflow-layout': adaptiveFooter }" class="modal-panel" role="dialog" aria-modal="true" aria-labelledby="copy-role-title" tabindex="-1" @keydown.tab="trapFocus">
...
const props = defineProps<{ adaptiveFooter?: boolean; visible: boolean; itcode: string; error?: string }>()
...
.modal-actions { display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px; border-top: 1px solid var(--color-border-subtle); padding-top: 14px; }
...
.permission-overflow-layout .modal-actions { position: relative; z-index: 2; margin: 20px -24px -24px; padding: 12px 24px; background: var(--color-surface, #fff); box-shadow: none; }
.modal-panel.permission-overflow-layout.modal-content-overflowing > .modal-actions { position: sticky; bottom: -24px; }
