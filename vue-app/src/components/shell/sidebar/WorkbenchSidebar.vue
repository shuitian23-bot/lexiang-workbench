...
const basePocLogRecords = [
  {
    ...
    title: 'AI 助手任务与批量授权',
    changePoint: '将授权清单、执行进度和完成状态合并到同一任务卡，支持勾选本次只读操作后批量确认。',
    detail: '已授权及已完成的步骤在原任务中更新，不再重复堆叠确认卡。批量确认仅限当前任务中明确列出的同范围只读操作；导出、修改等操作单独确认，新增或变化的操作需重新选择。修正较早请求误处理最新请求的问题，切换会话、过期及历史恢复后不能继续使用旧确认。执行详情默认收起，失败与拒绝保持可见。当前工作台提供标明示例数据的交互预览，不执行真实命令、导出文件或修改业务数据；真实执行需由对应服务端逐项校验并接入。发布环境、人员、时间和版本分别以实际记录为准。',
    deployTargets: [],
    status: '发布状态以环境记录为准'
  },
  {
