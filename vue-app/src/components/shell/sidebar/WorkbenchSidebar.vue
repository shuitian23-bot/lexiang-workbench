...
const basePocLogRecords = [
  {
    ...
    title: 'AI 助手任务与批量授权',
    changePoint: '将授权和进度合并为 Skill 批量授权卡，普通查询完整展示执行步骤，一次授权即可继续本次执行。',
    detail: '移除勾选、全选、已选计数和逐项确认，只保留授权与拒绝。授权覆盖本次 Skill 已列明的查询、汇总、导出等步骤，状态在原卡更新；步骤及技术详情默认收起，单项不重复展示标题和说明。授权仍绑定当前会话、Skill 执行和请求版本，新增或变化的步骤不能沿用旧批准；原有权限和审批要求保持。修正较早请求误处理最新请求的问题，切换会话、过期及历史恢复后不能使用旧确认。当前工作台提供标明示例数据的交互预览，不执行真实命令、导出文件或修改业务数据；真实执行需由对应服务端校验并接入。发布环境、人员、时间和版本分别以实际记录为准。',
    deployTargets: [],
    status: '发布状态以环境记录为准'
  },
  {
