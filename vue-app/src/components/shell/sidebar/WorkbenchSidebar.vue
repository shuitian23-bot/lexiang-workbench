...
const basePocLogRecords = [
  {
    ...
    detail: '依据 2026-08-21 能力上下文受控更新方案，将线上状态、工作流状态和能力更新状态拆分管理；初始 Mock 在草稿、待审批、已审批待发布、已发布和已禁用五类生命周期中提供“有更新”示例，已驳回只保留主状态，不叠加能力更新状态和操作。列表状态区最多显示一个主状态和一个更新提示，操作列统一读取状态机操作码；存在待决策变化时只保留查看变化、更新和忽略更新，不再叠加编辑、审批、发布或启停操作。负责人操作严格按 Skill owner 判定，列表按钮和保存、提审写入入口都会校验 owner，管理员不会自动取得他人 Skill 的维护权限；草稿综合评分达到 0.80 且不存在待决策变化或失败扫描后才允许提交审核。补齐正在准备、更新失败、同任务重试、更新中继续编辑、撤回审核、审批驳回、发布以及更新中再次发现变化的处理；待审批选择更新会回到同一编辑版本草稿并重新扫描，连续变化按检测顺序排队，当前扫描失败时必须先在编辑页复用同一任务重试，不会被下一条变化覆盖。高风险忽略必须填写原因，普通增强变化原因选填，忽略仅关闭当前变化记录。更新版本发布时保留原禁用状态，完整刷新恢复初始 Mock；公共导航、右侧 Agent、其他业务页面和受保护运行文件保持不变。',
    status: '已更新 new 预览'
  },
  {
