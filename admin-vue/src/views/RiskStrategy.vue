  ...
  <div class="page-inner">
    <div class="page-header">
      <div>
        ...
        <div class="page-desc">策略列表、策略审核、异常地址/邮箱、策略黑白名单</div>
      </div>
          ...
          <tbody>
            ...
            <tr v-for="(row, idx) in filteredRows" :key="idx">
              ...
              <td class="action-cell">
                ...
                <button class="btn btn-sm btn-secondary" @click="toggleStatus(row)">
                  {{ row.statusClass === 'success' ? '停用' : '审核' }}
                </button>
              </td>
            </tr>
          </tbody>
...
const headers = ['策略名称', '场景', '风险类型', '处置动作', '状态', '命中量', '更新时间', '操作']

const allRows = ref([
  { name: '下单高危设备拦截', scene: '下单', riskType: '设备聚集', action: '禁止下单', status: '生效',   statusClass: 'success', hits: 8912, updatedAt: '2026-06-05 10:20' },
  { name: '优惠券异常领取',   scene: '优惠券', riskType: '批量领取', action: '禁止用券', status: '待审核', statusClass: 'warning', hits: 2184, updatedAt: '2026-06-05 09:12' },
])
...
function toggleStatus(row) {
  row.status = row.statusClass === 'success' ? '已失效' : '生效'
  row.statusClass = row.statusClass === 'success' ? 'danger' : 'success'
}
</script>
