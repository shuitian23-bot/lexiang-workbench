...
function hasBusinessIntent(text) {
  return /企业|公司|采购|商务|办公采购|批量|政企|政教|大企业|中小企业|服务器|工作站|云桌面|解决方案|ThinkPad|ThinkBook/i.test(String(text || ''));
}
...
module.exports = {
  ...
  parameters: {
    ...
    properties: {
      ...
      site_type: { type: 'string', enum: ['default', 'shop', 'b', 'biz'], description: '当前业务入口：shop=个人及家庭，b=中小企业，biz=政教及大企业。一般由系统传入。' },
      limit: { type: 'number', description: '返回数量，默认12，最多24' }
    },
    required: ['title']
  },
  execute: async ({ title, keyword, category, min_price, max_price, skus, site_type, limit }, context = {}) => {
    const siteType = normalizeSiteType(site_type || context.siteType);
    const originalText = String(context.userMessage || [title, keyword, category].filter(Boolean).join(' '));
    ...
    if (skus?.length) {
      ...
      if (siteType === 'shop' && !hasBusinessIntent(originalText)) {
        sql += ` AND LOWER(name) NOT LIKE '%thinkbook%' AND LOWER(name) NOT LIKE '%thinkpad%' AND LOWER(name) NOT LIKE '%thinkcentre%' AND LOWER(name) NOT LIKE '%thinkstation%'`;
        sql += ` AND name NOT LIKE '%昭阳%' AND name NOT LIKE '%开天%' AND name NOT LIKE '%启天%' AND name NOT LIKE '%扬天%' AND name NOT LIKE '%瑞天%'`;
      }
      sql += ` ORDER BY CASE
        WHEN ? = 'shop' AND name LIKE '%小新%' THEN 0
        WHEN ? = 'shop' AND LOWER(name) LIKE '%yoga%' THEN 1
        WHEN ? = 'shop' AND (name LIKE '%拯救者%' OR LOWER(name) LIKE '%legion%') THEN 2
        ELSE 6
      END, sort_order DESC, price ASC LIMIT ?`;
      params.push(siteType, siteType, siteType);
      params.push(limit);
