...
function renderAgentPermissionManager() {
  const state = getPermissionFlowState();
  const activeType = PERMISSION_REQUEST_TYPES.find(item => item.key === state.type) || PERMISSION_REQUEST_TYPES[0];
  const approvalPath = buildPermissionApprovalPath(state, activeType);
  const selectedScopes = PERMISSION_SCOPE_OPTIONS.filter(item => state.scopes.includes(item.key));
  const pendingCount = permissionFlowPendingCount(approvalPath);
  return `
    <div class="page-header skill-hub-header">
      <div>
        <div class="page-title">权限管理</div>
        <div class="page-desc">从业务场景发起权限申请，串联关联人确认、直线经理、业务审批人、系统审批人和后台执行。</div>
      </div>
      <div class="skill-hub-actions">
        <button class="btn btn-secondary" onclick="goPortalHome()">返回工作台</button>
        <button class="btn btn-primary" onclick="permissionFlowReset()">新建申请</button>
      </div>
    </div>

    <div class="permission-overview-strip">
      <section><span>当前申请单</span><b>${state.requestNo}</b><em>${state.submitted ? '已提交' : '草稿中'}</em></section>
      <section><span>申请类型</span><b>${activeType.title}</b><em>${activeType.risk}</em></section>
      <section><span>待处理节点</span><b>${pendingCount}</b><em>${state.statusLabel}</em></section>
      <section><span>已选权限</span><b>${selectedScopes.length}</b><em>菜单 / Skill / 数据</em></section>
    </div>

    <div class="permission-flow-layout" id="permission-flow-root">
      <section class="card permission-flow-main">
        <div class="card-header">
          <div>
            <div class="card-title">新建权限申请</div>
            <div class="permission-card-subtitle">按原型主链路组织：入口选择、申请填写、关联确认、审批流转、后台执行。</div>
          </div>
          <span class="permission-coverage-pill">${state.submitted ? state.statusLabel : '填写中'}</span>
        </div>

        <div class="permission-type-grid">
          ${PERMISSION_REQUEST_TYPES.map(item => `
            <button type="button" class="${item.key === state.type ? 'active' : ''}" onclick="permissionFlowSetType('${item.key}')">
              <b>${item.title}</b>
              <span>${item.desc}</span>
              <em>${item.risk}</em>
            </button>
        ...
        <div class="permission-application-grid">
          <div class="permission-form-card">
            <div class="permission-section-title">申请信息</div>
            <div class="permission-form-grid">
              <label>申请人<input value="${state.applicant}" readonly disabled></label>
              <label>手机号<input value="${state.mobile}" readonly disabled></label>
              <label>邮箱<input value="${state.email}" readonly disabled></label>
              <label>申请对象
                <select onchange="permissionFlowSetField('targetMode', this.value)">
                  <option value="self" ${state.targetMode === 'self' ? 'selected' : ''}>本人申请</option>
                  <option value="proxy" ${state.targetMode === 'proxy' ? 'selected' : ''}>代他人申请</option>
                </select>
              </label>
              <label>关联人 / 被申请人
                <div class="permission-inline-input">
                  <input value="${state.targetUser}" oninput="permissionFlowSetField('targetUser', this.value)" ${state.targetMode === 'self' ? 'disabled' : ''}>
                  <button class="btn btn-secondary btn-sm" onclick="showPermissionPicker('itcode')">选择</button>
                </div>
              </label>
              <label>直线经理<input value="${state.manager}" readonly disabled></label>
              <label>业务审批人
                <select onchange="permissionFlowSetField('businessApprover', this.value)">
                  ${PERMISSION_BUSINESS_APPROVERS.map(item => `<option value="${item.value}" ${state.businessApprover === item.value ? 'selected' : ''}>${item.label}</option>`).join('')}
                </select>
              </label>
              <label>系统审批人<input value="${state.systemApprover}" readonly disabled></label>
              <label class="wide">申请原因 / 开通需求
                <textarea oninput="permissionFlowSetField('reason', this.value)" placeholder="请详细描述业务需求，例如：需要联想商城商品上架能力、Query 明细导出、某业务域数据查看等。">${state.reason}</textarea>
              </label>
            </div>
          </div>

          <div class="permission-form-card">
            <div class="permission-section-title">选择申请权限</div>
            <div class="permission-scope-list">
              ${PERMISSION_SCOPE_OPTIONS.map(item => `
                <label class="${state.scopes.includes(item.key) ? 'checked' : ''}">
                  <input type="checkbox" ${state.scopes.includes(item.key) ? 'checked' : ''} onchange="permissionFlowToggleScope('${item.key}', this.checked)">
                  <span><b>${item.title}</b><em>${item.desc}</em></span>
                  <i>${item.type}</i>
                </label>
              `).join('')}
            </div>
            <div class="permission-form-actions">
              <button class="btn btn-secondary" onclick="showPermissionPicker('role')">角色筛选</button>
              <button class="btn btn-secondary" onclick="showPermissionPicker('data')">数据范围</button>
              <button class="btn btn-primary" onclick="permissionFlowSubmit()">提交申请</button>
            </div>
          </div>
        </div>
      </section>

      <aside class="card permission-flow-side">
        <div class="card-header">
          <div>
            <div class="card-title">申请单状态</div>
            <div class="permission-card-subtitle">模拟真实审批流转，可逐步确认/审批并更新执行结果。</div>
          </div>
        </div>
        <div class="permission-ticket-card">
          <span>${state.requestNo}</span>
          <b>${activeType.title}</b>
          <p>${state.reason || activeType.desc}</p>
          <div>${selectedScopes.map(item => `<em>${item.title}</em>`).join('') || '<em>暂未选择权限</em>'}</div>
        </div>
        <div class="permission-flow-timeline">
          ${approvalPath.map(step => renderPermissionFlowStep(step, state)).join('')}
        </div>
        <div class="permission-node-actions">
          ${renderPermissionCurrentActions(state, approvalPath)}
        </div>
      </aside>
    </div>

    <div class="permission-workbench-grid permission-workbench-grid-compact">
      <section class="card permission-request-panel">
        <div class="card-header">
          <div>
            <div class="card-title">审批队列</div>
            <div class="permission-card-subtitle">审批人视角查看待处理申请，处理结果会同步到申请链路。</div>
          </div>
        </div>
        <div class="permission-request-list">
          ${renderPermissionQueueCard('line', '直线经理待审批', state.manager, state.steps.manager, '确认申请人与被申请人关系、权限范围和开通周期。')}
          ${renderPermissionQueueCard('business', '业务负责人待审批', permissionBusinessApproverLabel(state.businessApprover), state.steps.business, '确认业务域、数据范围和高风险操作边界。')}
          ${renderPermissionQueueCard('system', '系统审批人待执行', state.systemApprover, state.steps.system, '审批通过后自动写入权限，并生成审计记录。')}
        </div>
      </section>
      ...
      <section class="card permission-request-panel">
        <div class="card-header">
          <div>
            ...
            <div class="permission-card-subtitle">用于 POC 演示后台执行和审计留痕。</div>
          </div>
        </div>
        <div class="permission-audit-list">
          ${renderPermissionAuditRows(state)}
        </div>
      </section>
    </div>
    ...
    <div class="permission-hub-table card">
      <div class="card-header">
        <div>
          <div class="card-title">权限对象配置</div>
          <div class="permission-card-subtitle">作为申请链路的底层配置，支撑菜单、功能、数据和 Skill 能力授权。</div>
        </div>
      </div>
      <table class="table">
        <thead><tr><th>权限对象</th><th>配置方式</th><th>申请入口</th><th>审批规则</th><th>执行方式</th><th>状态</th></tr></thead>
        <tbody>
          <tr><td><b>菜单权限</b><small>左侧菜单和子菜单可见范围</small></td><td>角色继承 + 个人补充</td><td>权限变更</td><td>直线经理 + 业务负责人</td><td>后台自动写入</td><td><span class="badge green">可申请</span></td></tr>
          <tr><td><b>功能权限</b><small>按钮、导出、发布、启停等动作</small></td><td>功能点绑定角色</td><td>权限变更</td><td>按风险等级追加系统审批</td><td>执行后留痕</td><td><span class="badge orange">需确认</span></td></tr>
          <tr><td><b>数据权限</b><small>组织、业务域、字段和导出范围</small></td><td>普通范围 / 自定义范围</td><td>权限变更</td><td>业务负责人并行审批</td><td>脱敏策略生效</td><td><span class="badge blue">有边界</span></td></tr>
          <tr><td><b>账号状态</b><small>创建、启用、禁用、重置密码</small></td><td>人员 ITCode + 组织关系</td><td>账号类申请</td><td>关联人确认 + BPO / 系统审批</td><td>后台自动执行</td><td><span class="badge green">已接入链路</span></td></tr>
        </tbody>
      </table>
    </div>
  `;
...
const PERMISSION_REQUEST_TYPES = [
  { key: 'change', title: '权限变更', desc: '为本人或他人申请菜单、功能、Skill 或数据权限。', risk: '标准审批', defaultScopes: ['menu-lead', 'query-export', 'skill-call'] },
  { key: 'create', title: '创建账号', desc: '为新用户初始化账号、组织、角色和基础菜单权限。', risk: '账号审批', defaultScopes: ['account-basic', 'menu-ops'] },
  { key: 'enable', title: '启用账号', desc: '恢复已禁用账号登录和原有权限，需确认启用原因。', risk: '账号审批', defaultScopes: ['account-enable'] },
  { key: 'disable', title: '禁用账号', desc: '高风险操作，代他人申请时必须关联人确认。', risk: '高风险', defaultScopes: ['account-disable'] },
  { key: 'reset', title: '重置密码', desc: '代替用户发起重置，需关联人确认和系统执行。', risk: '高风险', defaultScopes: ['account-reset'] }
];

const PERMISSION_SCOPE_OPTIONS = [
  { key: 'menu-lead', title: '企业客户管理菜单', desc: '线索看板、线索池、打分模型可见', type: '菜单' },
  { key: 'menu-ops', title: '乐享运营菜单', desc: '运营总览、Query、质量、流量和 GMV 分析', type: '菜单' },
  { key: 'query-export', title: 'Query 明细导出', desc: 'CSV 下载，默认脱敏并记录申请原因', type: '功能' },
  { key: 'skill-call', title: '右侧 Agent Skill 调用', desc: '可调用已开启技能包和知识库查询', type: 'Skill' },
  { key: 'data-search', title: '搜索后台数据范围', desc: '本团队 Query、来源、结果和质量指标', type: '数据' },
  { key: 'data-cert', title: '在职员工认证数据', desc: '认证结果、失败原因和审核压力数据', type: '数据' },
  { key: 'account-basic', title: '基础账号与角色', desc: '创建账号并绑定业务用户角色', type: '账号' },
  { key: 'account-enable', title: '启用账号', desc: '恢复登录和原绑定角色', type: '账号' },
  { key: 'account-disable', title: '禁用账号', desc: '关闭登录并冻结当前权限', type: '账号' },
  { key: 'account-reset', title: '重置密码', desc: '触发系统密码重置流程', type: '账号' }
];

const PERMISSION_BUSINESS_APPROVERS = [
...
function getPermissionFlowState() {
  if (!window.PERMISSION_FLOW_STATE) {
    const firstType = PERMISSION_REQUEST_TYPES[0];
    window.PERMISSION_FLOW_STATE = {
      requestNo: `AP-${new Date().toISOString().slice(2,10).replace(/-/g,'')}-001`,
      type: firstType.key,
      applicant: 'zhangrui32',
      mobile: '13800000000',
      email: 'zhangrui32@lenovo.com',
      targetMode: 'self',
      targetUser: 'zhangrui32',
      manager: 'sunll1',
      businessApprover: 'zhangxy43',
      systemApprover: 'sunzh4',
      reason: '需要查看企业客户管理和搜索后台数据，用于排查线索评分与 Query 质量问题。',
      scopes: [...firstType.defaultScopes],
      submitted: false,
      rejected: false,
      statusLabel: '草稿待提交',
      currentNode: 'draft',
      ...
      audit: ['草稿已创建，等待补充申请原因和权限范围。']
    };
  }
  return window.PERMISSION_FLOW_STATE;
}

function permissionFlowRender() {
  const container = document.getElementById('page-content') || document.querySelector('.content-section');
  if (container) container.innerHTML = renderAgentPermissionManager();
}

function permissionFlowSetType(type) {
  const nextType = PERMISSION_REQUEST_TYPES.find(item => item.key === type) || PERMISSION_REQUEST_TYPES[0];
  const state = getPermissionFlowState();
  state.type = nextType.key;
  state.scopes = [...nextType.defaultScopes];
  state.targetMode = ['disable', 'reset'].includes(nextType.key) ? 'proxy' : state.targetMode;
  state.targetUser = state.targetMode === 'self' ? state.applicant : 'wangxt8';
  state.submitted = false;
  state.rejected = false;
  state.statusLabel = '草稿待提交';
  state.currentNode = 'draft';
  state.steps = { applicant: 'done', contact: 'skip', manager: 'pending', business: 'pending', system: 'pending', execute: 'pending' };
  state.audit = [`已切换为${nextType.title}申请，待提交。`];
  permissionFlowRender();
}

function permissionFlowSetField(field, value) {
  const state = getPermissionFlowState();
  state[field] = value;
  if (field === 'targetMode') {
    state.targetUser = value === 'self' ? state.applicant : 'wangxt8';
  }
  permissionFlowRender();
}

function permissionFlowToggleScope(key, checked) {
  const state = getPermissionFlowState();
  state.scopes = checked ? Array.from(new Set([...state.scopes, key])) : state.scopes.filter(item => item !== key);
  permissionFlowRender();
}

function permissionFlowSubmit() {
  const state = getPermissionFlowState();
  const type = PERMISSION_REQUEST_TYPES.find(item => item.key === state.type) || PERMISSION_REQUEST_TYPES[0];
  const needContact = permissionNeedsContact(state, type);
  state.submitted = true;
  state.rejected = false;
  state.currentNode = needContact ? 'contact' : 'manager';
  state.statusLabel = needContact ? '待关联人确认' : '待直线经理审批';
  state.steps = {
    applicant: 'done',
    contact: needContact ? 'current' : 'skip',
    manager: needContact ? 'pending' : 'current',
    business: 'pending',
    system: 'pending',
    execute: 'pending'
  };
  state.audit = [
    `${state.applicant} 已提交 ${type.title} 申请。`,
    needContact ? `${state.targetUser} 需先确认关联申请。` : `${state.manager} 已收到直线经理审批待办。`
  ];
  permissionFlowRender();
  skillHubToast('权限申请已提交，审批链路已生成');
}

function permissionFlowApprove(node) {
  const state = getPermissionFlowState();
  if (!state.submitted || state.rejected) return;
  const nodeText = {
    contact: '关联人已确认',
    manager: '直线经理已通过',
    business: '业务审批人已通过',
  ...
  state.steps[node] = 'done';
  state.audit.unshift(nodeText[node] || '节点已处理');
  const order = ['contact', 'manager', 'business', 'system', 'execute'];
  const next = order.find(item => state.steps[item] === 'pending');
  if (next) {
    state.steps[next] = 'current';
    state.currentNode = next;
    state.statusLabel = permissionNodeStatusLabel(next);
  } else {
    state.currentNode = 'done';
    state.statusLabel = '已完成';
  }
  permissionFlowRender();
  skillHubToast(node === 'execute' ? '权限已生效，审计记录已生成' : '已处理当前审批节点');
}

function permissionFlowReject(node) {
  const state = getPermissionFlowState();
  state.rejected = true;
  state.currentNode = 'rejected';
  state.statusLabel = '已驳回';
  if (node && state.steps[node]) state.steps[node] = 'rejected';
  state.audit.unshift(`${permissionNodeName(node)} 已驳回，申请人可修改后重新提交。`);
  permissionFlowRender();
  skillHubToast('权限申请已驳回，可修改后重新提交');
}

function permissionFlowReset() {
  window.PERMISSION_FLOW_STATE = null;
  getPermissionFlowState();
  permissionFlowRender();
  skillHubToast('已新建权限申请草稿');
}

function permissionNeedsContact(state, type) {
  return state.targetMode === 'proxy' || ['disable', 'reset'].includes(type.key);
}

function buildPermissionApprovalPath(state, type) {
  const needsContact = permissionNeedsContact(state, type);
  return [
    { key: 'applicant', title: '申请人填写', owner: state.applicant, desc: '选择申请类型、权限范围和审批人', status: state.steps.applicant },
    { key: 'contact', title: '关联人确认', owner: state.targetUser, desc: '代他人或高风险账号操作需要确认', status: needsContact ? state.steps.contact : 'skip' },
    { key: 'manager', title: '直线经理审批', owner: state.manager, desc: '确认组织关系、申请必要性和有效期', status: state.steps.manager },
    { key: 'business', title: '业务审批人审批', owner: permissionBusinessApproverLabel(state.businessApprover), desc: '确认业务域、角色和数据范围', status: state.steps.business },
    { key: 'system', title: '系统审批人审批', owner: state.systemApprover, desc: '确认安全策略和执行条件', status: state.steps.system },
    { key: 'execute', title: '后台自动执行', owner: '权限服务', desc: '写入权限、同步菜单和审计留痕', status: state.steps.execute }
  ];
}

function permissionFlowPendingCount(path) {
  return path.filter(item => ['pending', 'current'].includes(item.status)).length;
}

function permissionBusinessApproverLabel(value) {
  return PERMISSION_BUSINESS_APPROVERS.find(item => item.value === value)?.label || value || '待选择';
}

function renderPermissionFlowStep(step) {
  return `
    <article class="${step.status}">
      ...
      <em>${permissionStepLabel(step.status)}</em>
    </article>
  `;
}

function renderPermissionCurrentActions(state, path) {
  if (!state.submitted) return '<button class="btn btn-primary" onclick="permissionFlowSubmit()">提交并生成审批流</button>';
  if (state.rejected) return '<button class="btn btn-primary" onclick="permissionFlowSubmit()">修改后重新提交</button>';
  const current = path.find(item => item.status === 'current');
  if (!current) return '<button class="btn btn-secondary" onclick="permissionFlowReset()">再发起一单</button>';
  if (current.key === 'execute') {
    return `<button class="btn btn-primary" onclick="permissionFlowApprove('execute')">执行并生效</button>`;
  }
  return `
    <button class="btn btn-secondary" onclick="permissionFlowReject('${current.key}')">驳回</button>
    <button class="btn btn-primary" onclick="permissionFlowApprove('${current.key}')">通过当前节点</button>
  `;
}

function renderPermissionQueueCard(node, title, owner, status, desc) {
  const isActionable = status === 'current';
  return `
    <article class="${status}" data-permission-node="${node}">
      <i class="${status === 'done' ? 'green' : status === 'current' ? 'blue' : status === 'rejected' ? 'red' : 'gray'}">${permissionStepLabel(status)}</i>
      <b>${title}</b>
      <p>${owner}</p>
      <div><span>${desc}</span></div>
      <footer>
        <button class="btn btn-sm btn-secondary" ${isActionable ? '' : 'disabled'} onclick="permissionFlowReject('${node}')">驳回</button>
        <button class="btn btn-sm btn-primary" ${isActionable ? '' : 'disabled'} onclick="permissionFlowApprove('${node}')">通过</button>
      </footer>
    </article>
  `;
}

function renderPermissionAuditRows(state) {
  const rows = state.audit.length ? state.audit : ['暂无执行记录。'];
  return rows.map((text, index) => `
    <article>
      <span>${index === 0 ? '刚刚' : `${index + 1} 分钟前`}</span>
      <b>${text}</b>
      <p>${state.requestNo} · ${state.statusLabel}</p>
    </article>
  `).join('');
}

function permissionStepLabel(status) {
  return {
    done: '已完成',
    current: '处理中',
    pending: '待处理',
...
function permissionNodeName(node) {
  return {
    contact: '关联人',
    manager: '直线经理',
    business: '业务审批人',
...
function permissionNodeStatusLabel(node) {
  return {
    contact: '待关联人确认',
    manager: '待直线经理审批',
    business: '待业务审批',
...
function showPermissionPicker(type) {
  const pickerMap = {
    itcode: {
      title: '选择关联人',
      desc: '按 ITCode、姓名或邮箱选择被申请人 / 关联人。',
      items: ['wangxt8 · 王欣彤 · 搜索运营', 'zhangrui32 · 张睿 · 乐享运营', 'liujy7 · 刘佳怡 · 企业客户管理']
    },
    role: {
      title: '角色筛选及选择',
      desc: '选择需要申请的角色模板，可叠加个人补充权限。',
      items: ['乐享运营 · 可查看运营分析和 Query', '企业客户运营 · 可查看线索和打分模型', '搜索运营 · 可维护词典和搜索配置']
    },
    data: {
      title: '数据范围选择',
      desc: '选择组织、业务域、字段级别和导出脱敏策略。',
      items: ['本团队数据 · 默认脱敏', '搜索后台 Query 明细 · 30 天有效', '在职员工认证数据 · 高敏字段脱敏']
    }
  };
  const data = pickerMap[type] || pickerMap.role;
  const overlay = document.createElement('div');
  overlay.className = 'permission-live-modal';
  overlay.innerHTML = `
    <div class="permission-dialog-box">
      <button class="permission-dialog-close" onclick="this.closest('.permission-live-modal').remove()">×</button>
      <h3>${data.title}</h3>
      <p>${data.desc}</p>
      <div class="permission-form-grid">
        <label>关键词<input placeholder="输入关键词搜索"></label>
        <label>状态<select><option>全部</option><option>启用</option><option>可申请</option></select></label>
      </div>
      <div class="permission-option-list">
        ${data.items.map((item, index) => `<label><input type="checkbox" ${index === 0 ? 'checked' : ''}> ${item}</label>`).join('')}
      </div>
      <footer>
        <button class="btn btn-secondary" onclick="this.closest('.permission-live-modal').remove()">取消</button>
        <button class="btn btn-primary" onclick="skillHubToast('已选择并带回申请单');this.closest('.permission-live-modal').remove()">确认</button>
      </footer>
    </div>
  `;
  document.body.appendChild(overlay);
