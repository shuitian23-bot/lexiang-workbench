...
function submitSkillClarifyMessage() {
  ...
  const stateNode = appendSkillConversationStates([
    ['thinking', 'running', '理解补充需求', '正在结合基础配置、能力上下文和当前澄清记录判断缺口。'],
    ['tool_call', 'pending', '检查约束条件', '准备检查定时触发、CSV 导出、审批风险和权限边界。'],
    ['streaming', 'pending', '组织澄清回复', '将把缺口转换为下一轮澄清问题。']
  ]);
  chat.scrollTop = chat.scrollHeight;
  setTimeout(() => {
    stateNode?.remove();
    appendSkillClarifyAssistant('已记录。请继续确认这个需求是否需要固定时间触发、是否允许导出 CSV，以及是否有需要审批的高风险操作。', [
      ['thinking', 'done', '理解补充需求', '已写入当前 Skill 需求上下文。'],
      ['tool_call', 'done', '检查约束条件', '发现仍需确认定时触发、CSV 导出和审批风险。'],
      ['streaming', 'done', '组织澄清回复', '已生成下一轮可确认的问题。'],
      ['follow_up', 'blocked', '等待用户追问确认', '请继续补充时间频率、导出权限和高风险动作边界。']
    ]);
  }, 1800);
}

function appendSkillClarifyAssistant(message, states = []) {
  const chat = document.getElementById('skill-clarify-chat');
  ...
  if (states?.length) appendSkillConversationStates(states);
  chat.scrollTop = chat.scrollHeight;
}

function appendSkillConversationStates(items) {
  const chat = document.getElementById('skill-clarify-chat');
  if (!chat || !items?.length) return null;
  const wrap = document.createElement('div');
  wrap.className = 'skill-chat-ai skill-conversation-states';
  wrap.setAttribute('aria-label', 'AI 会话状态');
  wrap.innerHTML = items.map(([kind, status, title, detail]) => `
    <div class="skill-conversation-state is-${kind} status-${status}">
      <span class="skill-state-icon" aria-hidden="true">${skillConversationStateIcon(kind)}</span>
      <span class="skill-state-body">
        <span class="skill-state-title-row"><b>${escapeSkillChatHtml(title)}</b><em>${skillConversationStateStatus(status)}</em></span>
        ${detail ? `<span class="skill-state-detail">${escapeSkillChatHtml(detail)}</span>` : ''}
      </span>
      ${status === 'running' ? '<span class="skill-state-dots" aria-hidden="true"><i></i><i></i><i></i></span>' : ''}
    </div>
  `).join('');
...
function skillConversationStateStatus(status) {
  return ({
    pending: '等待中',
    running: '进行中',
    done: '已完成',
...
function skillConversationStateIcon(kind) {
  const icons = {
...
function escapeSkillChatHtml(value) {
          ...
          <div class="skill-create-panel active" data-skill-create-panel="config">
            ...
            <div class="skill-create-form">
              ...
              <div class="skill-create-field">
                ...
                <select id="skill-create-menu" required><option>在职员工管理</option><option>乐享运营</option><option>GEO 看板</option><option>企业客户管理</option></select>
              </div>
          ...
          <div class="skill-create-panel" data-skill-create-panel="clarify">
            ...
            <div class="skill-clarify-layout">
              <div class="skill-chat-sim" id="skill-clarify-chat">
                ...
                <div class="skill-chat-ai skill-conversation-states" aria-label="AI 会话状态">
                  <div class="skill-conversation-state is-tool_result status-done"><span class="skill-state-icon" aria-hidden="true">${skillConversationStateIcon('tool_result')}</span><span class="skill-state-body"><span class="skill-state-title-row"><b>澄清结果已汇总</b><em>已完成</em></span><span class="skill-state-detail">已形成触发场景、文件结构、输出格式、测试用例和权限边界。</span></span></div>
                  <div class="skill-conversation-state is-confirm status-blocked"><span class="skill-state-icon" aria-hidden="true">${skillConversationStateIcon('confirm')}</span><span class="skill-state-body"><span class="skill-state-title-row"><b>等待进入草稿生成</b><em>待确认</em></span><span class="skill-state-detail">点击下一步后再生成 Skill 草稿，不直接发布或修改线上状态。</span></span></div>
                </div>
  ...
  return `
    ...
    <div class="permission-hub-grid">
      <section class="permission-hub-card">
        ...
        <div><em>在职员工管理</em><em>乐享运营</em><em>GEO 看板</em><em>企业客户管理</em></div>
      </section>
      <section class="permission-hub-card">
        <span>SKILL</span>
        <b>Skill 权限</b>
