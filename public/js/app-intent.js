...
(function (root) {
  ...
  function matchControl(text) {
    ...
    // 「最优款下单吧」「买最好的那款」：最优/最好/最合适 = 乐享最推荐那款，本地闭环不走官方下单 Skill（真机反馈）
    if (/^(?!.*(不下单|别下单|先不|不要|不买|取消|暂不)).{0,8}(最优|最好|最合适|最值|最推荐)的?[那这]?[款个台]?.{0,4}(下单|购买|买)吧?[!！。]?$/.test(_t)) return { op: "buy_recommended", target: "", msg: "好的，正在为你下单最推荐的那款。" };
