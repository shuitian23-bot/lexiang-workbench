...
;/* public/leaip0/assets/frontend/js/pages/shop/inline-22.smb-products-v121.js */
...
(function registerDeviceSceneHall() {
  ...
  // Complete product destinations for every SMB carousel frame.
  // Products absent from P0 use their verified official Lenovo detail pages.
  var businessProductDetails = {
    "高效办公": [{sku:"1047099"}, {sku:"1056625"}, {sku:"1044588"}],
    "移动差旅": [{sku:"1044932"}, {url:"https://item.lenovo.com.cn/product/1044584.html"}, {url:"https://biz.lenovo.com.cn/pd/1043218.html"}],
    "专业设计": [{url:"https://b.lenovo.com.cn/product/1047701.html"}, {sku:"1053767"}, {sku:"1037590"}],
    "会议协作": [{url:"https://biz.lenovo.com.cn/product/0120O010103.html"}, {sku:"1037592"}, {url:"https://biz.lenovo.com.cn/pd/1048383.html"}],
    "成长型团队": [{sku:"1056625"}, {sku:"1054831"}, {sku:"1052987"}]
  };
  ...
  function renderScene(index) {
    ...
    if (scene.cards && recommendationCards.length) {
      recommendationCards.forEach(function (card, cardIndex) {
        ...
        card.removeAttribute("data-smb-product-url");
        if (profile === "business") {
          var destination = businessProductDetails[scene.name][cardIndex];
          card.removeAttribute("data-brand-ask");
          card.removeAttribute("data-open-product");
          if (destination.sku) card.dataset.openProduct = destination.sku;
          else card.dataset.smbProductUrl = destination.url;
        } else {
          card.removeAttribute("data-open-product");
  ...
  if (profile === "business") {
    recommendationCards.forEach(function(card) {
      card.addEventListener("click", function(event) {
        var url = card.dataset.smbProductUrl;
        if (!url) return; // Native data-open-product handler opens the internal detail tab.
  ...
  if (profile === "business" && sceneAction) {
    sceneAction.addEventListener("click", function(event) {
      event.preventDefault();
      event.stopImmediatePropagation();
      window.__lxBridge?.sendChat("咨询" + scenes[currentIndex].name + "解决方案");
    }, true);
  }
  renderScene(0);
})();
...
;/* public/leaip0/assets/components/smb-hero-reference-v1/carousel.js */
(function () {
  'use strict';
