...
(function () {
  ...
  if (!document.querySelector('[data-checkout-answer-v68-style]')) {
    var answerStyle = document.createElement('style');
    answerStyle.dataset.checkoutAnswerV68Style = 'true';
    answerStyle.textContent = '.lx-checkout-benefit-list{display:grid;gap:10px;margin:14px 0 4px}.lx-checkout-benefit-row{display:grid;grid-template-columns:18px minmax(0,1fr);gap:8px;align-items:center;color:#2f2a31;font-size:13px;line-height:20px}.lx-checkout-benefit-check{display:grid;width:16px;height:16px;place-items:center;border-radius:50%;background:#e6f7ef;color:#19a365}.lx-checkout-benefit-check svg{display:block;width:12px;height:12px}.lx-checkout-benefit-name{font-weight:600}';
  ...
  function benefitNames() {
    return ['国补焕新专享券', '商品平台满减券', '联想会员折扣券'];
  }
  function benefitMarkup() {
    ...
    return '<div class="lx-checkout-benefit-list" aria-label="已使用优惠">' + benefitNames().map(function (name) {
      return '<div class="lx-checkout-benefit-row"><span class="lx-checkout-benefit-check" aria-hidden="true">' + check + '</span><span class="lx-checkout-benefit-name">' + name + '</span></div>';
    }).join('') + '</div>';
  }
  ...
  function normalizePurchaseAnswer(body, product) {
    ...
    copy.insertAdjacentHTML('afterend', benefitMarkup());
  ...
  function appendFulfillmentChat(product, mode) {
    ...
    var answer = '<div class="lx-payment-confirm-copy"><p>已为你按<strong>' + modeLabel + '</strong>方式重新生成【' + esc(product.name || '该商品') + '】的待支付订单，请确认门店、收货信息和优惠后继续。</p></div>' + benefitMarkup() + card;
    var assistantMessage = !fullscreen && window.__lxAgentAPI && window.__lxAgentAPI.addAiMessage ? window.__lxAgentAPI.addAiMessage(answer) : null;
    if (!assistantMessage) {
      assistantMessage = document.createElement('div');
      assistantMessage.className = fullscreen ? 'lxfd-msg-ai lx-chat-skin' : 'lx-p0-message msg ai lx-chat-skin';
      assistantMessage.innerHTML = fullscreen ? '<div class="lxfd-ai-body">' + answer + '</div>' : '<div class="ai-body">' + answer + '</div>';
      host.appendChild(assistantMessage);
    }
    if (window.__lxState) {
