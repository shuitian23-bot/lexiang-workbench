(function () {
  ...
  function detail(payload) {
    ...
    return `<section class="lx-recruitment-page lx-job-detail" data-job-detail="${escape(job.id)}"><button type="button" class="lx-job-back lx-job-secondary" data-job-back>返回招聘列表</button><header class="lx-job-detail-head"><div><div class="lx-job-tags"><span>联想招聘</span><span>${escape(job.direction)}</span><span>全职</span></div><h2>${escape(job.title)}</h2>${job.officialTitle !== job.title ? '<p class="lx-job-official-title">官网职位名称：'+escape(job.officialTitle)+'</p>' : ''}<p class="lx-job-lead">${escape(job.summary)}</p><p class="lx-job-source">职位编号 ${escape(job.reqId)} · ${escape(job.posted)} 发布 · ${escape(checkedAt)} 核对</p></div><div class="lx-job-apply-area"><a class="lx-job-primary" data-job-apply href="${escape(job.applyUrl)}" target="_blank" rel="noopener noreferrer">一键应聘</a><span>在联想招聘官网登录后提交简历</span></div></header><dl class="lx-job-facts"><div><dt>工作地点</dt><dd>${escape(job.city)}</dd></div><div><dt>经验要求</dt><dd>${escape(job.experience)}</dd></div><div><dt>学历要求</dt><dd>${escape(job.education)}</dd></div><div><dt>薪资信息</dt><dd>官网未公开</dd></div></dl><div class="lx-job-detail-body"><main><section class="lx-job-section"><h3>岗位职责</h3>${bullets(job.duties)}</section><section class="lx-job-section"><h3>任职要求</h3>${bullets(job.requirements)}</section><section class="lx-job-section"><h3>申请方式</h3><p>点击“一键应聘”进入该职位的联想官方申请入口，登录或创建招聘账户后，按官网要求填写资料并提交简历。</p><a href="${escape(job.url)}" target="_blank" rel="noopener noreferrer">查看官方完整职位描述</a></section></main><aside><section class="lx-job-fit"><h3>乐享推荐理由</h3><p>${escape(job.reason)}</p><small>AI 整理，供方向参考</small></section><section class="lx-job-section"><h3>应聘准备</h3><p>结合岗位职责准备相关项目经历，在简历中说明你的工作范围、方法和成果。</p><p>具体流程及所需材料以官方申请页面为准。</p></section></aside></div><p class="lx-p0-disclaimer">${disclaimer}</p></section>`;
  }
