...
(function (root) {
  ...
  const STYLE = `
    .lx-voice-btn{display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;
      padding:0;border:none;border-radius:50%;background:transparent;color:#625b68;cursor:pointer;
      flex:0 0 auto;transition:background .16s ease,color .16s ease,transform .12s ease;position:relative}
    .lx-voice-btn:hover{background:rgba(50, 29, 57, 0.08);color:#4d144a}
    .lx-voice-btn:active{transform:scale(.94)}
    .lx-voice-btn .lx-voice-icon{width:19px;height:19px;display:block}
    .lx-voice-btn.unsupported{opacity:.45}
    .lx-voice-btn.recording{color:#fff;background:#e42b20;box-shadow:0 0 0 0 rgba(77, 20, 74, 0.48);
      animation:lxVoicePulse 1.25s ease-out infinite}
    .lx-voice-btn.thinking{color:#fff;background:#a262d7}
    @keyframes lxVoicePulse{0%{box-shadow:0 0 0 0 rgba(77, 20, 74, 0.48)}
    ...
    .lx-voice-tip{position:fixed;z-index:99999;max-width:260px;padding:9px 13px;border-radius:10px;
      background:#252525;color:#fff;font-size:13px;line-height:1.5;box-shadow:0 6px 20px rgba(155, 142, 182, 0.62);
    ...
    .lx-voice-confirm{position:fixed;z-index:99999;display:flex;align-items:center;gap:10px;flex-wrap:wrap;
      padding:10px 14px;border-radius:13px;background:#fff;border:1px solid #e2ddeb;
      box-shadow:0 10px 30px rgba(146, 86, 214, 0.16);font-size:13.5px;color:#252525;max-width:min(94vw,560px);
      opacity:0;transform:translateY(6px);transition:opacity .18s ease,transform .18s ease}
    .lx-voice-confirm.show{opacity:1;transform:translateY(0)}
    .lx-voice-confirm .lx-vc-text{flex:0 0 100%;color:#4d144a;line-height:1.55;word-break:break-word;padding-right:92px}
    .lx-voice-confirm .lx-vc-text b{color:#101010;font-weight:600}
    /* 倒计时挪右上角小字，两个按钮一起靠右挨着（手在右侧就近点，真机反馈） */
    .lx-voice-confirm .lx-vc-tip{position:absolute;top:9px;right:14px;color:#979797;white-space:nowrap;font-size:12px}
    .lx-voice-confirm .lx-vc-cancel{margin-left:auto}
    .lx-voice-confirm .lx-vc-count{display:inline-block;min-width:15px;text-align:center;font-weight:700;color:#e42b20}
    ...
    .lx-voice-confirm .lx-vc-send{background:#4d144a;color:#fff}
    .lx-voice-confirm .lx-vc-cancel{background:#f3edf6;color:#625b68}
  ...
  const MIC_ICON_SRC = currentScriptSrc
    ? new URL("../../../icons/composer-mic.svg", currentScriptSrc).href
    : "../icons/composer-mic.svg";
