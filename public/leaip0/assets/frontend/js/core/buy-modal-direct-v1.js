(() => {
  ...
  const visibleDetailProduct = (button) => {
    const root = button.closest('.product-detail, .detail-page, .detail-main, .lx-product-detail') || document;
    const name = String(root.querySelector('.detail-title, .product-title, h1, h2')?.textContent || '联想小新 Air 13').trim();
    ...
    const originalPrice = prices[0] || 7299;
    const discount = /小新\s*Air\s*13/i.test(name) ? 400 : 0;
    const image = root.querySelector('.detail-product-image, .detail-visual img, img');
    return { name, originalPrice, discount, price: Math.max(0, originalPrice - discount), image_url: imageForProduct(name, image?.getAttribute('src') || '') };
  };
  ...
  const openOrderModal = (product) => {
    ...
    const showPriceDetail = () => {
      ...
      dialog.innerHTML = `<button class="lx-buy-direct-close" type="button" aria-label="关闭">×</button><header class="lx-buy-sub-head"><button class="lx-order-edit-back" type="button" data-price-back>‹</button><h2>价格明细</h2></header><div class="lx-buy-price-list"><div class="lx-buy-price-line"><strong>商品金额</strong><span class="amount">¥${product.originalPrice.toLocaleString('zh-CN')}.00</span><span></span></div><div class="lx-buy-price-line"><strong>运费</strong><span class="amount">＋¥0.00</span><span></span></div><div class="lx-buy-price-line"><strong>优惠券</strong><span class="discount">−¥${couponDiscount.toLocaleString('zh-CN')}.00</span><span class="arrow">›</span></div><div class="lx-buy-price-line"><strong>其他优惠</strong><span class="discount">−¥0.00</span><span class="arrow">›</span></div><div class="lx-buy-price-line is-clickable" data-open-coupon><strong>优惠码</strong><span class="muted">请输入优惠码</span><span class="arrow">›</span></div></div><div class="lx-buy-sub-footer"><div class="lx-buy-sub-total"><b>¥${product.price.toLocaleString('zh-CN')}.00</b><span>节省了：<em>¥${couponDiscount.toLocaleString('zh-CN')}.00</em></span></div><button class="primary" type="button" data-price-confirm>确定</button></div>`;
    ...
    const showOrderEdit = () => {
      ...
      dialog.innerHTML = `<header class="lx-order-edit-head"><button class="lx-order-edit-back" type="button" data-order-back>‹</button><h2>修改订单</h2><button class="lx-buy-direct-close" type="button" aria-label="关闭">×</button></header><div class="lx-order-edit-body"><div class="lx-order-address"><img src="/assets/icons/order-address-location.svg" alt="" aria-hidden="true"><b>1　演示地址可在订单中修改收货信息</b><button type="button" data-address-edit>修改地址</button></div><section class="lx-order-edit-section"><h3>选择支付方式</h3><div class="lx-order-channel-grid"><button class="lx-order-channel is-active" type="button" data-order-channel="online"><strong>在线支付</strong><small>快捷支付或分期支付</small></button><button class="lx-order-channel" type="button" data-order-channel="corporate"><strong>对公支付</strong><small>需开具增值税专用发票</small></button></div><div data-online-payment><div class="lx-order-subtitle">快捷支付</div><div class="lx-order-quick-grid">${quickHtml}</div><div class="lx-order-subtitle"><span>分期支付</span><small>*手续费以支付平台实际收取为准</small></div>${providersHtml}</div></section><section class="lx-order-edit-section"><h3>订单补充信息</h3><div class="lx-order-form"><label for="lxOrderNote">订单备注</label><input id="lxOrderNote" data-order-note value="${escapeHtml(orderState.note)}"><label for="lxOrderCustomer">客户编码</label><div class="lx-order-combobox"><input id="lxOrderCustomer" data-order-customer value="${escapeHtml(orderState.customerCode)}" readonly aria-haspopup="listbox" aria-expanded="false"><div class="lx-order-code-menu" data-order-code-menu role="listbox" hidden>${codes.map((code) => `<button type="button" role="option" data-order-code="${code}">${code}</button>`).join('')}</div></div></div></section><button class="lx-order-invoice" type="button" data-order-invoice><strong>发票信息</strong><span>${escapeHtml(orderState.invoice)}</span><i class="lx-order-chevron"></i></button></div><footer class="lx-order-edit-footer"><button type="button" data-order-save>确认</button></footer>`;
      dialog.querySelector('[data-order-back]').addEventListener('click', (event) => { event.stopPropagation(); showOrder(); });
    };
    const showInvoiceEdit = () => {
      const options = ['电子普通发票（个人）','电子普通发票（单位）','增值税专用发票'];
      dialog.className = 'lx-buy-direct-dialog lx-order-edit-dialog lx-invoice-dialog';
      dialog.innerHTML = `<header class="lx-order-edit-head"><button class="lx-order-edit-back" type="button" data-invoice-back>‹</button><h2>发票信息</h2><button class="lx-buy-direct-close" type="button" aria-label="关闭">×</button></header><div class="lx-order-edit-body"><section class="lx-order-edit-section"><h3>选择发票类型</h3><div class="lx-order-channel-grid">${options.map((option) => `<button class="lx-order-channel${orderState.invoice === option ? ' is-active' : ''}" type="button" data-invoice-option="${option}"><strong>${option}</strong><small>订单提交前可修改</small></button>`).join('')}</div></section><section class="lx-order-edit-section"><h3>收票信息</h3><div class="lx-order-form"><label>发票抬头</label><input value="${orderState.invoice.includes('个人') ? '个人' : '联想（北京）有限公司'}"><label>收票人手机</label><input value="135****9879"><label>收票人邮箱</label><input value="ziyu@lenovo.com"></div></section></div><footer class="lx-order-edit-footer"><button type="button" data-invoice-save>保存</button></footer>`;
    ...
    modal.addEventListener('click', (event) => {
      ...
      if (target.closest('[data-open-coupon]')) return showCoupon();
      ...
      if (invoiceOption) { orderState.invoice = invoiceOption.dataset.invoiceOption; return showInvoiceEdit(); }
      if (target.closest('[data-invoice-save]')) return showOrderEdit();
  ...
  window.addEventListener('click', (event) => {
    const button = event.target.closest?.('.detail-primary');
    if (!button || button.dataset.bizQuote || !/下单/.test(button.textContent || '')) return;
    event.preventDefault();
    event.stopImmediatePropagation();
  ...
  if (previewMode) {
    const showPreview = () => {
      ...
      if (previewMode === 'coupon') document.querySelector('[data-open-coupon]')?.click();
