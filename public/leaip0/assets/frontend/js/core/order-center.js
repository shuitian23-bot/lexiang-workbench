(function () {
        ...
        function renderDetailProducts(order) {
          return '<div class="lx-order-detail-products">' + (order.items || []).map(function (item, index) {
            ...
            var orderMeta = index === 0 ? '<div class="lx-order-detail-meta">订单编号 ' + escapeHtml(order.id) + ' · ' + escapeHtml(order.typeLabel) + '</div>' : "";
