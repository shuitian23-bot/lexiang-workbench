(function () {
        ...
        function detailMarkup(order) {
          ...
          return '<header class="lx-order-detail-head"><button class="lx-order-back" type="button" data-order-back><img src="../icons/arrow-left.svg" alt="">返回订单列表</button><div class="lx-order-detail-title"><h1>订单详情</h1><p>订单编号 ' + escapeHtml(order.id) + ' · ' + escapeHtml(order.typeLabel) + '</p></div><span class="lx-order-detail-status">' + escapeHtml(order.status) + '</span></header>' +
