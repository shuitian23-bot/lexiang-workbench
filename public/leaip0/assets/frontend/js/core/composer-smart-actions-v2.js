(function () {
  ...
  var readyLayoutSignature = "";
  var readyLayoutSince = 0;
  ...
  function assistantGenerationComplete() {
    if (document.body.classList.contains("lx-agent-generating")) return false;
    if (window.__lxState && window.__lxState.sending) return false;
    var messages = document.querySelectorAll(".lx-p0-message.ai, .lxfd-msg.ai, .lxfd-msg-assistant");
    if (!messages.length) return true;
    var last = messages[messages.length - 1];
    return !last.matches(".loading,.streaming") &&
      !last.querySelector(".loading-line,.lx-generating,.typing-cursor,.streaming,[aria-busy=\"true\"]");
  }

  function recommendationPageComplete(content) {
    if (!content || content.getAttribute("aria-busy") === "true") return false;
    if (content.classList.contains("is-generating-tab")) return false;
    if (content.querySelector('.lx-page-generating.is-show,[aria-busy="true"],.skeleton,.is-loading,.loading')) return false;
    if (!assistantGenerationComplete()) return false;

    var rows = recommendationRows(content);
    if (!rows.length) return false;
    var images = Array.from(content.querySelectorAll(".reco-row img,.lx-reco-poc-row img"));
    if (images.some(function (image) { return !image.complete || !image.naturalWidth; })) return false;

    var layout = rows.map(function (row) {
      var card = row.closest(".reco-row,.lx-reco-poc-row") || row;
      var rect = card.getBoundingClientRect();
      return [Math.round(rect.width), Math.round(rect.height), Math.round(rect.top)].join("x");
    }).join("|");
    if (!layout || layout !== readyLayoutSignature) {
      readyLayoutSignature = layout;
      readyLayoutSince = Date.now();
      return false;
    }
    return Date.now() - readyLayoutSince >= 1000;
  }
  ...
  function recommendationListReady() {
    ...
    if (!signature || signature === baselineRecommendationSignature || !hasRecommendationHeading(content) || !recommendationPageComplete(content)) {
      readyCandidateSignature = "";
      readyCandidateSince = 0;
      return false;
    }
    ...
    return Date.now() - readyCandidateSince >= 700;
  }
