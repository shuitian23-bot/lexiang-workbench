...
(function () {
  ...
  function renderChoices(mask, metas, selectedSku) {
    ...
    colorsBox.innerHTML = colors.length ? colors.map(function (color) {
      return '<button type="button" class="lx-product-config-color' + (color === selected.color ? ' is-selected' : '') + '" data-lx-config-color="' + esc(color) + '">' + esc(color) + '</button>';
    }).join("") : '<div class="lx-product-config-empty">该系列暂无独立颜色选项</div>';
  }
  ...
  document.addEventListener("click", function (event) {
    ...
    if (colorButton) {
      ...
      if (target) renderChoices(mask, metas, target.variant.sku);
      return;
    }
    if (event.target.closest("[data-lx-config-save]")) {
      ...
      var discountTotal = Array.isArray(context.pending.benefits) ? context.pending.benefits.reduce(function (sum, benefit) { return sum + Math.abs(Number(benefit.amount || 0)); }, 0) : 0;
      context.pending.price = Math.max(0, context.pending.original_price - discountTotal);
