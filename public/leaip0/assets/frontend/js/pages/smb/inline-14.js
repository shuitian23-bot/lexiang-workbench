...
/* === Stable SMB category tabs === */
(function () {
  var labels = ["推荐", "企业会员权益", "企业定制", "积分商城", "门店"];
  ...
  var syncing = false;
  var selectedIndex = 0;
  var content = tabs.closest(".content");
  var making = content && content.querySelector(":scope > .lx-shop-making");
  if (content && !making) {
    making = document.createElement("section");
    making.className = "lx-shop-making";
    making.setAttribute("aria-live", "polite");
    making.innerHTML = '<p class="lx-shop-making-copy">正在制作中...</p>';
    tabs.insertAdjacentElement("afterend", making);
  }
  ...
  function applySelectedTab() {
    ...
    if (content) content.classList.toggle("lx-shop-making-state", selectedIndex !== 0);
    if (making) making.setAttribute("aria-hidden", selectedIndex === 0 ? "true" : "false");
  }
  ...
  tabs.addEventListener("click", function (event) {
    ...
    var index = Array.from(tabs.querySelectorAll(":scope > button")).indexOf(button);
...
(function () {
  // 中小企业「推荐」楼层统一由 core/app.js 从正式货盘接口渲染：
  // /api/products?site=b、/api/site/feed?site=b，以及 /api/products/:sku 商详。
  // 禁止此处的历史演示数据覆盖接口返回的真实商品。
  if (document.body && document.body.dataset.page === "business") return;
  ...
  var catalog = [
    ["ThinkPad", "T14 2025 锐龙 AI 7 PRO AI 工程师本", "AMD 锐龙 AI 7 PRO 350｜32G｜512G", "¥ 11,799", "../img/thinkpad.jpg"],
    ["ThinkPad", "P16v 2025 英特尔酷睿 Ultra 7 设计本", "酷睿 Ultra 7｜32G｜1TB｜专业显卡", "¥ 19,799", "../img/thinkpad.jpg"],
    ["ThinkPad", "P16s 2025 酷睿 Ultra 7 AI 元启版", "酷睿 Ultra 7｜32G｜1TB｜16英寸", "¥ 16,299", "../img/thinkpad.jpg"],
    ["ThinkPad", "P16 2026 酷睿 Ultra 7 移动工作站", "酷睿 Ultra 7｜RTX 专业显卡｜1TB", "¥ 34,799", "../img/thinkpad.jpg"],
    ["拯救者", "创世 18英寸个人 AI 移动工作站", "酷睿 Ultra 9｜RTX 5090｜锻造碳", "¥ 79,999", "../img/thinkpad.jpg"],
    ["拯救者", "Y9000P《黑神话：悟空》联名定制版", "i9-14900HX｜RTX 4060｜2.5K屏", "¥ 11,820", "../img/thinkpad.jpg"],
    ["ThinkPad", "P1 2026 酷睿版移动工作本", "CTO 可选配置｜专业创作工作站", "¥ 28,500", "../img/thinkpad.jpg"],
    ["ThinkPad", "T1g 酷睿 Ultra 7 移动工作站", "酷睿 Ultra 7｜RTX 5060｜4K屏", "¥ 30,299", "../img/thinkpad.jpg"],
    ["ThinkBook", "Plus Hybrid 酷睿 Ultra 7 二合一电脑", "平板笔记本二合一｜高效协同", "¥ 17,999", "../img/thinkpad.jpg"],
    ["ThinkPad", "X1 Carbon AI 2024 英特尔酷睿版", "酷睿 Ultra 7｜轻薄商务｜长续航", "¥ 13,999", "../img/thinkpad.jpg"],
  ...
  [["ThinkPad", 0, 10], ["ThinkBook", 10, 20]].forEach(function (floorInfo) {
...
(function () {
  // 线上五入口统一由 app.js 管理商详结果页与标签；此处仅供静态模板预览。
  ...
  function upsertDetailTab(product, card) {
    if (!detailTabs.some(function (tab) { return tab.id === "site:business"; })) {
      detailTabs.push({ id: "site:business", label: "中小企业" });
    }
    ...
    if (detailTabs.length > 8) {
      var removable = detailTabs.findIndex(function (tab) { return tab.id !== "site:business" && tab.id !== activeDetailTab; });
  ...
  function openDetailInSharedTab(card) {
    ...
    sharedDetailCards.set(sku, card);
  ...
  /* 共享标签再次激活时，复用首次打开所使用的 SMB 详情渲染器。 */
  window.__lxRestoreTemplateProductDetail = function (tab) {
    if (!tab || !String(tab.sku || "").startsWith("template-")) return false;
    var card = sharedDetailCards.get(tab.sku);
    if (!card && tab.product && tab.product.name) {
      card = Array.from(document.querySelectorAll(".lx-floor-product-card")).find(function (item) {
        return item.dataset.detailTitle === tab.product.name;
      });
      if (card) sharedDetailCards.set(tab.sku, card);
    }
    if (!card) return false;
    activeDetailTab = tabIdFor(detailProduct(card));
    openDetail(card, { skipTabUpdate: true });
    syncDetailConversationSelection();
    return true;
  };
  ...
  document.addEventListener("click", function (event) {
    ...
    if (event.target.closest(".detail-back")) {
      event.preventDefault();
      event.stopImmediatePropagation();
      activateDetailTab("site:business");
      return;
    }
...
(function () {
        function enterShopSplit() {
          ...
          document.querySelectorAll(".main-nav [data-page]").forEach(function (button) {
            button.classList.toggle("active", button.dataset.page === "business");
          });

          var business = document.querySelector('.main-nav [data-page="business"]');
          if (business && !business.dataset.shopTemplateBooted) {
            business.dataset.shopTemplateBooted = "1";
            business.click();
          }
...
(function () {
        ...
        function ensureTopicChip() {
          ...
          var label = nav.getAttribute("data-current-label") || "中小企业：新对话";
          label = label.replace(/^(?:首页|个人及家庭)：/, "中小企业：");
...
/* 2026-08-05 修复：本地文件打开时，灵动岛标题同步逻辑
      ...
      (function () {
        ...
        function syncCompactTitle() {
          ...
          if (query && name) {
            var label = "中小企业：" + shortTitle(query);
            // 同步全屏灵动岛
