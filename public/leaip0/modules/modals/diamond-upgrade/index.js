/* 升级钻石会员 — P0 business implementations. */
if (!window.__p0Modules.installed["modals/diamond-upgrade"]) {
window.__p0Modules.installed["modals/diamond-upgrade"]=true;

/* Business: openEnterpriseMemberAuth */
window.__p0Modules.factories["modals/diamond-upgrade#openEnterpriseMemberAuth:2eccabe7ce57be50bc218bc6"]=function(__p0Scope){return (function openEnterpriseMemberAuth() {
    ...
    form.addEventListener("submit", event => {
      ...
      try { localStorage.setItem(__p0Scope.applicationKey, JSON.stringify({status: "pending", submittedAt: Date.now()})); } catch {}
      (0,__p0Scope.close)(mask);
      (0,__p0Scope.appendSubmittedMessage)();
      window.__lxToast?.("钻石会员认证申请已提交，请留意审核结果");
    });
    mask.addEventListener("click", event => { if (event.target === mask || event.target.closest("[data-modal-close]")) (0,__p0Scope.close)(mask); });
