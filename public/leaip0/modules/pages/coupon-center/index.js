/* 我的优惠券 — P0 business implementations. */
if (!window.__p0Modules.installed["pages/coupon-center"]) {
window.__p0Modules.installed["pages/coupon-center"]=true;

/* public/leaip0/assets/frontend/js/core/member-coupon-center-v1.js */
window.__p0Modules.sources["u0940425517a453a5"]=function(){
(function () {
  'use strict';
...
/* Business: ss */
window.__p0Modules.factories["pages/coupon-center#ss:ce4f61aae4ce511292dc0494"]=function(__p0Scope){"use strict";return (function ss(){(0,__p0Scope.Qt)("coupon","优惠券","asset:coupons","tab")}); };

/* Business: renderCouponRecords */
window.__p0Modules.factories["pages/coupon-center#renderCouponRecords:de9224aeaed9bcbda9bea8dd"]=function(__p0Scope){"use strict";return (function renderCouponRecords(records) {
    var statusLabels = { available: "待使用", used: "已使用", expired: "已过期" };
    return '<div class="leai-coupon-list">' + records.map(function (coupon) {
      var opened = Boolean(__p0Scope.state.couponDetailsOpen[coupon.id]);
      var available = coupon.status === "available";
      return '<article class="leai-coupon-card is-' + (0,__p0Scope.escapeHtml)(coupon.status) + '" data-coupon-card="' + (0,__p0Scope.escapeHtml)(coupon.id) + '" data-asset-record-status="' + (0,__p0Scope.escapeHtml)(coupon.status) + '"><div class="leai-coupon-value"><strong>' + (0,__p0Scope.escapeHtml)(coupon.value) + '</strong><small>会员优惠券</small></div><div class="leai-coupon-copy"><span class="leai-coupon-state">' + statusLabels[coupon.status] + '</span><h3>' + (0,__p0Scope.escapeHtml)(coupon.title) + '</h3><p>' + (0,__p0Scope.escapeHtml)(coupon.validFrom) + '–' + (0,__p0Scope.escapeHtml)(coupon.validTo) + '</p><div class="leai-coupon-actions"><button type="button" data-coupon-detail-toggle="' + (0,__p0Scope.escapeHtml)(coupon.id) + '" aria-expanded="' + opened + '">详细说明 <img src="' + __p0Scope.icons.expand + '" alt=""></button>' + (available ? '<button class="leai-coupon-products" type="button" data-coupon-products="' + (0,__p0Scope.escapeHtml)(coupon.id) + '">可用商品</button>' : '') + '</div><p class="leai-coupon-detail" data-coupon-detail aria-hidden="' + (!opened) + '"' + (opened ? '' : ' hidden') + '>' + (0,__p0Scope.escapeHtml)(coupon.description) + '</p></div></article>';
    }).join("") + '</div>';
  }); };

/* Business: couponProductsPage */
window.__p0Modules.factories["pages/coupon-center#couponProductsPage:afcd78f414362ab3a3e1f8ee"]=function(__p0Scope){"use strict";return (function couponProductsPage(couponId) {
    var coupon = __p0Scope.couponCatalog.find(function (item) { return item.id === couponId; }) || __p0Scope.couponCatalog[0];
    var products = coupon.productSkus.map(function (sku) { return __p0Scope.couponProductCatalog[sku]; }).filter(Boolean);
    var rows = products.map(function (product) {
      return '<article class="leai-coupon-product" data-coupon-product="' + (0,__p0Scope.escapeHtml)(product.sku) + '"><img src="' + product.image_url + '" alt=""><div><span>适用优惠券</span><h2>' + (0,__p0Scope.escapeHtml)(product.name) + '</h2><p>' + (0,__p0Scope.escapeHtml)(product.description) + '</p><strong>¥' + product.price.toLocaleString() + '</strong></div><div class="leai-coupon-product-actions"><button class="lx-p0-btn" type="button" data-coupon-product-open="' + (0,__p0Scope.escapeHtml)(product.sku) + '">查看详情</button><button class="lx-p0-btn primary" type="button" data-coupon-product-buy="' + (0,__p0Scope.escapeHtml)(product.sku) + '">立即购买</button></div></article>';
    }).join("");
    return '<section class="leai-page" data-coupon-products-page="' + (0,__p0Scope.escapeHtml)(coupon.id) + '" aria-labelledby="leaiCouponProductsTitle"><header class="leai-page-header"><div><div class="leai-page-title-row"><button class="leai-page-back" type="button" data-secondary-back="asset:coupons" aria-label="返回优惠券"><img src="' + __p0Scope.icons.next + '" alt=""></button><h1 class="leai-page-title" id="leaiCouponProductsTitle">可用商品</h1></div><p class="leai-page-desc">以下商品可使用“' + (0,__p0Scope.escapeHtml)(coupon.title) + '”，实际优惠以结算页为准。</p></div></header><div class="leai-coupon-product-list">' + rows + '</div><p class="leai-member-disclaimer">当前商品与优惠关系为 Mock 演示，价格、库存、适用范围与最终优惠以商城结算结果为准。</p></section>';
  }); };
}
