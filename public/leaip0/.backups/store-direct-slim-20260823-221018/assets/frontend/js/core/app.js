  ...
  return Array.from(parts.slice(0, 4).join("｜") || source).slice(0, 44).join("");
    ...
    if (spec && state.page === "business") {
      ...
      spec.title = fullSpec;
        ...
        // POC 占位流程：详情页主下单按钮先进入对话，正式下单能力后续接入。
        function lxStartOrderPlaceholder(product = state.currentProduct) {
          if (!product) return toast("请先选择商品");
          const item = normalizeProduct(product);
          addMessage("user", `我要立即下单 ${item.name}`);
          addMessage("assistant", "下单流程等待制作...");
...
function openOrderDetail(orderId) {
  ...
  lxOpenInfoTab("order-detail", "订单详情", html);
        ...
        async function lxRunDeviceActionBridge(kind, device) {
          ...
          const bridgeKey = isBind ? "__lxPendingDeviceBindBridge" : "__lxPendingDeviceWarrantyBridge";
          const bridge = window[bridgeKey];
          const name = current.name || (isBind ? "小新 Pro 16" : "拯救者 Y7000P");
          state.sending = true;
          const lines = [isBind ? "联想乐享正在校验设备订单与当前 Lenovo ID" : "联想乐享正在核验设备保障状态与可购买节点"];
          const skills = new Set();
          const skillName = isBind ? "设备资产绑定" : "保修商品推荐";
          const ai = addMessage("ai loading", "", renderSkillTrace(lines, { collapsed:false, foldable:false, skillCount:0 }));
          const body = lxEnsureAiBody(ai);
          const paint = () => { body.innerHTML = renderSkillTrace(lines, { collapsed:false, foldable:false, skillCount:skills.size }); };
          ...
          try {
            await wait(420);
            lines.push(isBind ? `已识别：${name} · 联想官方订单待绑定` : `已定位：${name} · ${current.warranty || "当前保障信息已读取"}`);
            paint(); await wait(520);
            skills.add(`Skill(${skillName})`);
            lines.push(`联想乐享官方 SKILL：正在调用 Skill(${skillName})`);
            paint(); await wait(760);
            lines[lines.length - 1] = `联想乐享官方 SKILL：Skill(${skillName}) 已完成`;
            const copy = isBind
              ? `绑定成功！**${name}** 已加入当前 Lenovo ID，设备信息与保障服务已同步，可在右侧“我的设备”列表中查看详情。`
              : `已根据 **${name}** 的型号、购买时间和当前保障节点，匹配到 3 款可购买保修商品，涵盖一年整机延保、两年整机延保及延保上门服务升级。建议结合使用年限、服务地区与预算选择，具体价格及适用范围以实时校验为准。`;
            await lxAnimateAiFinal(ai, mdLite(copy));
            const finalBody = lxEnsureAiBody(ai);
            finalBody.insertAdjacentHTML("afterbegin", renderSkillTrace(lines, { collapsed:true, foldable:true, skillCount:skills.size }));
            if (!isBind) finalBody.insertAdjacentHTML("beforeend", renderPageCta({ title:"查看推荐保修商品", desc:`3 款可购买方案 · 已关联${name}`, attr:'data-lx-warranty-result-card aria-label="推荐保修商品已在右侧展开"' }));
            ai.classList.remove("loading");
            await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
            bridge?.source?.postMessage({ type:isBind ? "lexiang:device-bind-success" : "lexiang:device-warranty-success", deviceId:current.id || (isBind ? "xiaoxinpro16" : "legiony7000p") }, location.origin);
          } catch (error) {
            bridge?.source?.postMessage({ type:isBind ? "lexiang:device-bind-failed" : "lexiang:device-warranty-failed", deviceId:current.id }, location.origin);
            throw error;
          } finally {
            window[bridgeKey] = null; state.sending = false;
            try { window.__lxSaveConversationNow(); } catch (_e) {}
          }
        }
        ...
        async function lxRunUnifiedEducationAuthAnswer(kind = "college") {
          ...
          try {
            ...
            paint();
            const copy = "**教育认证**可用于核验在校生、教师或高考生身份，并解锁对应的教育专享价格与会员权益。请根据你的真实身份选择认证方式，按页面提示填写资料；提交前可核对**适用范围、有效期和所需材料**，最终结果以正式身份核验信息为准。";
            ai._raw = copy;
            await lxAnimateAiFinal(ai, mdLite(copy));
            const finalBody = lxEnsureAiBody(ai);
            finalBody.insertAdjacentHTML("afterbegin", renderSkillTrace(lines, { collapsed: true, foldable: true, skillCount: skills.size }));
  ...
  if (!["personal", "business", "enterprise"].includes(page)) { grid.hidden = false; box.hidden = true; return; }
        ...
        window.addEventListener("message", (event) => {
          ...
          if (payload.type === "lexiang:device-bind-query" && payload.device) {
            ...
            sendChat(payload.query || ("一键绑定" + payload.device.name));
            return;
          }
          if (payload.type === "lexiang:device-warranty-query" && payload.device) {
            window.__lxPendingDeviceWarrantyBridge = { source:event.source, device:payload.device };
            sendChat(payload.query || ("为" + payload.device.name + "推荐可购买的保修商品"));
          }
        });
