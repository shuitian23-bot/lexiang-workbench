...
function _qLineSeries(dates, series, colors, yAxisOpts) {
  return {
    ...
    series: series.map((s, i) => ({
      ...
      areaStyle: s.area ? { color: new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:colors[i]+'33'},{offset:1,color:colors[i]+'05'}]) } : undefined,
    }))
  };
}
...
function renderSatisfactionTrend(data, dates) {
  ...
  ch.setOption({
    ...
    legend: { data: ['点踩case', '低满意度case'], bottom: 0, textStyle: { color: '#646a73', fontSize: 10 } },
    ...
    series: [
      { name: '点踩case', type: 'bar', stack: 'total', data: data.daily.map(d => d.thumbdown_count), itemStyle: { color: '#e2001a' }, barWidth: '55%' },
      { name: '低满意度case', type: 'bar', stack: 'total', data: data.daily.map(d => d.low_sat_count), itemStyle: { color: '#ff7d00' } },
    ]
  });
}
...
function renderRateTrend(data, dates) {
  ...
  ch.setOption({
    ...
    legend: { data: ['点踩率', '低满意率'], bottom: 0, textStyle: { color: '#646a73', fontSize: 10 } },
    grid: { left: 50, right: 50, top: 10, bottom: 36 },
    xAxis: { type: 'category', data: dates, axisLabel: { color: '#8f959e', fontSize: 9, rotate: 30 }, axisLine: { lineStyle: { color: '#e5e6eb' } } },
    yAxis: [
      { type: 'value', name: '点踩率', axisLabel: { color: '#e2001a', fontSize: 10, formatter: '{value}%' }, splitLine: { lineStyle: { color: '#e5e6eb', type: 'dashed' } }, nameTextStyle: { color: '#e2001a', fontSize: 10 } },
      { type: 'value', name: '低满意率', axisLabel: { color: '#ff7d00', fontSize: 10, formatter: '{value}%' }, splitLine: { show: false }, nameTextStyle: { color: '#ff7d00', fontSize: 10 } },
    ],
    series: [
      { name: '点踩率', type: 'line', data: tdRates, smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { width: 2, color: '#e2001a' }, itemStyle: { color: '#e2001a' } },
      { name: '低满意率', type: 'line', yAxisIndex: 1, data: lsRates, smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { width: 2, color: '#ff7d00' }, itemStyle: { color: '#ff7d00' } },
    ]
  });
}
...
function renderCsThumbdownTrend(data, dates) {
  ...
  ch.setOption({
    ...TH, tooltip: { trigger: 'axis' },
    legend: { data: ['点踩case'], bottom: 0, textStyle: { color: '#646a73', fontSize: 10 } },
    grid: { left: 50, right: 16, top: 10, bottom: 36 },
    xAxis: { type: 'category', data: dates, axisLabel: { color: '#8f959e', fontSize: 9, rotate: 30 }, axisLine: { lineStyle: { color: '#e5e6eb' } } },
    yAxis: { type: 'value', axisLabel: { color: '#8f959e', fontSize: 10 }, splitLine: { lineStyle: { color: '#e5e6eb', type: 'dashed' } } },
    series: [{ name: '点踩case', type: 'line', data: data.daily.map(d => d.cs_thumbdown), smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { width: 2, color: '#e2001a' }, itemStyle: { color: '#e2001a' }, areaStyle: { color: new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'#e2001a33'},{offset:1,color:'#e2001a05'}]) } }]
  });
}
...
function renderFirstTokenByIntent() {
  ...
  ch.setOption({
    ...
    series: [{ type: 'bar', data: sorted.map(s => s.avg), itemStyle: { color: '#3370ff', borderRadius: [0,3,3,0] }, barWidth: '55%', label: { show: true, position: 'right', formatter: '{c}ms', fontSize: 10, color: '#646a73' } }]
  });
}
...
function renderQaFirstToken(data, dates) {
  ...
  ch.setOption({
    ...TH, tooltip: { trigger: 'axis' },
    legend: { data: ['avg', 'p90', 'p99'], bottom: 0, textStyle: { fontSize: 10, color: '#646a73' } },
    ...
    series: [
      { name: 'avg', type: 'line', data: data.daily.map(d => d[prefix + 'avg']), smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { width: 2, color: '#3370ff' }, itemStyle: { color: '#3370ff' }, areaStyle: { color: new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'#3370ff33'},{offset:1,color:'#3370ff05'}]) } },
      { name: 'p90', type: 'line', data: data.daily.map(d => d[prefix + 'p90']), smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { width: 2, color: '#ff7d00' }, itemStyle: { color: '#ff7d00' } },
      { name: 'p99', type: 'line', data: data.daily.map(d => d[prefix + 'p99']), smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { width: 2, color: '#e2001a' }, itemStyle: { color: '#e2001a' } },
    ]
  });
}
...
function renderSessionWaitTrend(data, dates) {
  ...
  ], ['#3370ff']), yAxis: { axisLabel: { color: '#8f959e', fontSize: 10, formatter: '{value}s' } } });
}
...
function renderFirstTokenTrend(data, dates) {
  ...
  ].map((s, i) => ({ ...s, data: data.daily.map(d => [d.first_token_avg, d.first_token_p90, d.first_token_p95, d.first_token_p99][i]) })), ['#3370ff', '#34c724', '#ff7d00', '#e2001a']), yAxis: { axisLabel: { color: '#8f959e', fontSize: 10, formatter: '{value}ms' } } });
}
...
function renderFirstTokenNoCacheTrend(data, dates) {
  ...
  ch.setOption({ ...TH, ..._qLineSeries(dates, [
    { name: 'avg' }, { name: 'p90' }, { name: 'p95' }, { name: 'p99' },
  ].map((s, i) => ({ ...s, data: data.daily.map(d => [d.first_token_nocache_avg, d.first_token_nocache_p90, d.first_token_nocache_p95, d.first_token_nocache_p99][i]) })), ['#722ed1', '#34c724', '#ff7d00', '#e2001a']), yAxis: { axisLabel: { color: '#8f959e', fontSize: 10, formatter: '{value}ms' } } });
}
...
function renderAnswerWaitTrend(data, dates) {
  ...
  ch.setOption({
    ...TH, tooltip: { trigger: 'axis' },
    legend: { data: [label + '平均等待'], bottom: 0, textStyle: { color: '#646a73', fontSize: 10 } },
    grid: { left: 50, right: 16, top: 10, bottom: 36 },
    ...
    series: [{ name: label + '平均等待', type: 'line', data: filtered.daily.map(d => d[key]), smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { width: 2, color: '#3370ff' }, itemStyle: { color: '#3370ff' }, areaStyle: { color: new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'#3370ff33'},{offset:1,color:'#3370ff05'}]) } }]
  });
}
...
function renderSceneFailure(data, dates) {
  ...
  ch.setOption({
    ...
    series: [{ type: 'bar', data: sorted.map(s => s.count).reverse(), itemStyle: { color: function(p) { return p.value > 10 ? '#e2001a' : p.value > 5 ? '#ff7d00' : '#3370ff'; }, borderRadius: [0,3,3,0] }, barWidth: '55%', label: { show: true, position: 'right', fontSize: 10, color: '#646a73' } }]
  });
}
...
function renderNetScoreTrend(data, dates) {
  ...
  ch.setOption({
    ...TH, tooltip: { trigger: 'axis' },
    legend: { data: ['净评价值'], bottom: 0, textStyle: { color: '#646a73', fontSize: 10 } },
    grid: { left: 50, right: 16, top: 10, bottom: 36 },
    xAxis: { type: 'category', data: dates, axisLabel: { color: '#8f959e', fontSize: 9, rotate: 30 }, axisLine: { lineStyle: { color: '#e5e6eb' } } },
    yAxis: { type: 'value', axisLabel: { color: '#8f959e', fontSize: 10, formatter: '{value}%' }, splitLine: { lineStyle: { color: '#e5e6eb', type: 'dashed' } } },
    series: [{ name: '净评价值', type: 'line', data: netScores, smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { width: 2, color: '#3370ff' }, itemStyle: { color: '#3370ff' }, areaStyle: { color: new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'#3370ff33'},{offset:1,color:'#3370ff05'}]) } }]
  });
}
