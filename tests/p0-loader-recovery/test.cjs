...
function env(kind){
 const nodes=new Map(),requests=[],timers=new Map();let seq=0,mounts=0;
 class Element{
 ...
 const document={getElementById:id=>nodes.get(id)||null,querySelector:()=>null,querySelectorAll:()=>[],createElement:tag=>new Element(tag),head:{appendChild(node){nodes.set(node.id,node);if(node.tag==='script')requests.push(node);return node}}};
 ...
 let completed=0;
 return {nodes,requests,timers,window,host,state,root,
...
async function test(kind,name,fn){try{await fn(env(kind));results.push({kind,name,pass:true})}catch(error){results.push({kind,name,pass:false,error:error.message})}}
(async()=>{
