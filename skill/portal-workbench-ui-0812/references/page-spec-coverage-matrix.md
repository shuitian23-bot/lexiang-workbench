...
当前实现范围以 `lexiang-new-0812` 的以下文件为准：
...
Figma 04 是 133 个 UAT 页面快照的走查证据，检查依据是 Figma 01–03 与本 skill 已吸收的内容槽合同。它不是当前 0812 Vue 项目的路由清单，也不是正确样式源。
...
| 订单管理 / 协议采购单管理 | `/order/purchase-orders/:id` · `OrderPurchaseOrdersView.vue` · O1 | T1 · C1/C2/C3/C4/C9 | V1 · mock/service 数据，真实 API 与权限待接入 | S1；PM 0812 POC 新需求 | [VA-0](page-visual-acceptance.json) |
...
| 页面 | 归属 | 主合同 | 当前状态 | 视觉验收 |
|---|---|---|---|---|
| AI 动态报告 | `TempTabView.vue` · O4 | T3 + V3 · C1/C4/C9 · E6 | S4 | [VA-0](page-visual-acceptance.json)：0728 稳定演示 payload 仅在外部历史档案保留，0812 待复验 |
| 登录 | `/login` | 非普通内容槽；遵守品牌、表单、错误、键盘和响应式合同 | 认证专项 | 不计入本轮内容槽视觉基线 |
| WIP / 未知路由 | `:pathMatch(.*)*` | 非业务页型；必须说明未开放并提供返回路径 | 路由兜底，不得作为正式页面 | 不计入本轮内容槽视觉基线 |

### 3.4 当前复验状态与历史视觉证据

- 当前对象：`lexiang-new-0812/vue-app` 的 35 个稳定路由内容槽页面和 1 个 AI 动态报告页。
- 当前状态：所有页面保持 VA-0；尚未为 0812 重新完成宽屏、Agent 最大态、交互、数据状态、无障碍和工程联合验收。
- 历史证据：0728 的页面截图、动态报告截图和旧测量记录已外置到逻辑档案 `历史需求日志/20260806_设计Skill视觉证据精简/`，只用于结构、密度和历史问题对比，不随当前 Skill 分发。
- 历史记录曾同时出现“28 页通过、6 页溢出”与页面全部标为 VA-R2 的矛盾，因此 0812 不继承其通过状态。
...
其余 127 个 04 页面属于更广的 UAT 后台业务范围；它们仍受本 skill 的页型和组件规范覆盖，但不是当前 0812 路由。新增或迁回这些页面时，应先以第 5 节定位历史证据，再按新需求登记到第 3 节，而不是直接复制 UAT 快照。
