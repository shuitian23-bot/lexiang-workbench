...
- KPI 组合例外：页头到 KPI 12px、卡间 14px、KPI 到下一模块 16px
...
| `--sidebar-width-collapsed` | `58px` | 侧栏收起，仅图标；与当前 0803 壳层一致 |
...
当前项目骨架：
- 左侧栏：可收起，默认 168px
- 顶栏：56px，承载面包屑、静态页签、AI 助手开关；当前 0803 Vue 联想门户工作台默认隐藏全局搜索和亮/暗模式切换入口
...
当前 0803 版本不展示全局搜索和亮/暗模式切换入口。若未来明确恢复，必须沿用顶栏现有 icon-only 方形按钮、tooltip 和可访问状态规范。
