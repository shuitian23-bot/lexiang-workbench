...
当前实现范围以 `lexiang-new-0803` 的以下文件为准：
...
Figma 04 是 133 个 UAT 页面快照的走查证据，检查依据是 Figma 01–03 与本 skill 已吸收的内容槽合同。它不是当前 0803 Vue 项目的路由清单，也不是正确样式源。
...
专项变体只能叠加在一个主基础页型上：V1 详情/审核、V2 多步流程、V3 AI 报告、V4 门户首页、V5 日志/技术内容。V2 参考 `content-workflow-template.html`；V3 参考 `content-report-detail-template.html`。
...
- O4：Vue AI 结果视图组件，内容由运行时 payload 生成；当前实现名 `TempTabView.vue` 属于历史代码标识，不代表第二排动态页签 UI。
...
- E6：AI 动态报告正文和来源 payload 可变；报告壳、层级、状态和页签行为固定。
...
机器可读记录见 `page-visual-acceptance.json`，截图使用其相对路径。每次复核必须更新 `capturedAt / review / captures / findings / remainingStates`；只有自动一致性检查通过后才能回写本矩阵状态。
...
| 页面 | 归属 | 主合同 | 当前状态 | 视觉验收 |
|---|---|---|---|---|
| AI 动态报告 | `TempTabView.vue` · O4 | T3 + V3 · C1/C4/C9 · E6 | S4 | [VA-0](page-visual-acceptance.json)：仅保留 0728 稳定演示 payload 历史双态证据，0803 待复验 |
| 登录 | `/login` | 非普通内容槽；遵守品牌、表单、错误、键盘和响应式合同 | 认证专项 | 不计入本轮内容槽视觉基线 |
| WIP / 未知路由 | `:pathMatch(.*)*` | 非业务页型；必须说明未开放并提供返回路径 | 路由兜底，不得作为正式页面 | 不计入本轮内容槽视觉基线 |

### 3.4 当前复验状态与历史视觉证据

- 当前对象：`lexiang-new-0803/vue-app` 的 34 个稳定路由内容槽页面和 1 个 AI 动态报告页。
- 当前状态：所有页面保持 VA-0；尚未为 0803 重新完成宽屏、Agent 最大态、交互、数据状态、无障碍和工程联合验收。
- 历史证据：`visual-evidence/2026-07-27/` 与 `visual-evidence/2026-07-28/` 来自 0728 运行版本，只用于结构、密度和历史问题对比。
- 历史记录曾同时出现“28 页通过、6 页溢出”与页面全部标为 VA-R2 的矛盾，因此 0803 不继承其通过状态。
...
其余 127 个 04 页面属于更广的 UAT 后台业务范围；它们仍受本 skill 的页型和组件规范覆盖，但不是当前 0803 路由。新增或迁回这些页面时，应先以第 5 节定位历史证据，再按新需求登记到第 3 节，而不是直接复制 UAT 快照。
